...
**项目状态**：**v2.0 PRD 锁定执行 + v1.5.4 收尾 (2026-05-01)** — v2.0 启动**完整 HIPS 改造**（依据 [HIPS Readiness Assessment](docs/review/2026-05-01-hips-readiness-assessment.md) 评估"当前 HIPS 70%"，目标 GA 时 90%）：5 项能力（用户规则 / 三态决策 + Critical 锁 / 行为序列 beta / 规则引擎抽象 / 进程上下文）+ Phase A/B 拆分 + GA Week 12 不变 + 6 个新 ADR-020~025 + §9 三条新硬约束 #14/#15/#16，详见 [PRD v2.0](docs/prd/sieve-prd-v2.0.md) + [CHANGELOG v2.0](docs/changelog/CHANGELOG.md#v20-hips-readiness---2026-05-01)。同时 v1.5.4 完成：修 Week 4 P0 非流式 JSON 入站绕过漏洞 + OpenAI stream=false bug；累计入站规则 70 + 测试数据集 1951（含 55 条**真实攻击复现**：Ledger 2023 / Multichain $126M / Lazarus Op99 / CVE-2025-6514），PRD §9 #7 实测：**Critical FP 0.00% + Attack Recall 99.71% + 公开攻击拦截率 94.5%**。Week 2 起 OUT-01~11 出站规则上线，BIP39 SHA-256 校验，release 二进制 9.0 MB。repo 保持 private 至 Week 12 GA，见 [ADR-011](docs/design/ADR-011-private-until-ga.md)。
