...
**项目状态**：v1.5.4 完成 (2026-05-01) — **修 Week 4 标记 P0 的非流式 JSON 入站绕过漏洞**（v1.5.x 70 条入站规则在 JSON 响应模式此前实际拦截率 0%；OpenAI 协议默认 stream=false，意味着 OpenAI 入站规则从未生效过）。本 patch 后 v1.5.x 累计 70 条入站规则在所有响应模式生效。同时 v1.5.1~v1.5.3 累计：入站规则 35 → 70（针对"用户最怕五件事"+ BIP39 inbound second-pass + Windows PowerShell pipe / CVE-2025-6514），测试数据集 296 → 1951 条（含 55 条**已公开发生过的真实攻击复现**：Ledger 2023 / Multichain $126M / Lazarus Op99 / CVE-2025-6514），PRD §9 #7 硬约束实测：**Critical FP 0.00%（0/1070）+ Attack Recall 99.71%（694/696）+ 公开攻击拦截率 94.5%（52/55）**。详见 [CHANGELOG v1.5.4](docs/changelog/CHANGELOG.md#v154-non-streaming-json-inbound-fix---2026-05-01) + [SECURITY.md](SECURITY.md#历史-advisories)。Week 2 起 OUT-01~11 出站规则上线，BIP39 SHA-256 校验，release 二进制 9.0 MB。repo 保持 private 至 Week 12 GA，见 [ADR-011](docs/design/ADR-011-private-until-ga.md)。
