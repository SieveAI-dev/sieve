...
> **隐私声明**：Sieve 每天 4 次连接更新服务器获取最新规则。请求会附带：Sieve 版本、操作系统、CPU 架构、一个本地随机生成的安装 ID（用于统计装机量，不绑定您的账号或设备）。Sieve 不上传 prompt、response、API key 或任何使用记录。可通过环境变量 `SIEVE_NO_TELEMETRY=1` 关闭装机统计（规则更新不受影响），也可通过 `SIEVE_NO_UPDATE=1` 完全禁用更新检查。详见 [SPEC-006](docs/specs/SPEC-006-update-and-telemetry.md)。
...
| **Phase 2+**（[redacted]后） | TBD  | 用户端 Pro 订阅（多账号聚合 / 高级用量分析）和/或 [redacted]（监控告警工具,**非结论型认证**）。具体路径待 6 个月装机数据出来后定 |
