...
**项目状态**：v1.5.1 规则集扩充完成 (2026-05-01) — 入站规则 35 → 57 条（+22 条针对"用户最怕五件事"：签名/转账/.env/私钥/shell RCE），测试数据集 296 → 1896 条，PRD §9 #7 硬约束实测通过：**Critical FP 0.00%（0/1070）+ Attack Recall 97.13%（676/696）**。详见 [CHANGELOG v1.5.1](docs/changelog/CHANGELOG.md#v151-rule-expansion---2026-05-01)。Week 2 起 OUT-01~11 出站规则上线，BIP39 SHA-256 校验，release 二进制 9.0 MB。repo 保持 private 至 Week 12 GA，见 [ADR-011](docs/design/ADR-011-private-until-ga.md)。
