...
[![Status: alpha](https://img.shields.io/badge/status-0.1.0--alpha-orange.svg)](#项目状态)

> **完全本地运行的 LLM 流量安全代理——不可逆动作前的最后一道闸。**

Sieve 是一个完全本地运行的 LLM 流量安全代理（Rust 单二进制），夹在你的 AI 编码 agent（Claude Code / OpenClaw / Hermes / Codex CLI）和上游模型 API（Anthropic / OpenAI / 中转站）之间。它对双向流量做检测——出站脱敏（redacting secrets），入站拦截危险工具调用（fail-closed）——在不可逆动作（签名、转账、部署）之前强制插入一个认知摩擦的瞬间。专为 crypto-native 开发者打造。

```mermaid
flowchart LR
    A["AI agent<br/>(Claude Code / OpenClaw / Hermes / Codex CLI)"] <--> S["Sieve<br/>(本地 · 127.0.0.1)"]
...
1. **站位在 LLM 流量层**——Sieve 看得到完整的请求/响应字节流，钱包安全产品与 DLP 都够不到这一层。
2. **本地推理 + 边界明确的更新通道**——检测 100% 本地，零云依赖。
3. **Crypto 专项检测**——密钥/助记词/私钥格式（含 BIP39 SHA-256 checksum 验证、WIF 与 BIP-32 扩展私钥经 Base58Check 校验）以及链上地址替换检测，专为 crypto-native 工作流打造。
4. **双向检测 + fail-closed**——Critical 在任何模式下都不可关闭。

### 能力一览

- **双向检测**——出站脱敏（自动就地改写、不弹窗、状态栏 5s 通知）+ 入站拦截危险工具调用（fail-closed + 人工确认）。
- **四路由对等**——所有入站检测覆盖四类内容路由：Anthropic SSE / Anthropic JSON / OpenAI SSE / OpenAI JSON。功能绝不会只挂流式而静默漏掉非流式流量。
- **Crypto 私钥格式**——API 密钥与高熵密钥、BIP39 助记词（SHA-256 checksum 验证）、Bitcoin WIF 与 BIP-32 扩展私钥（xprv）经 Base58Check 校验。
- **地址替换检测**——跟踪会话内出现过的 EVM 地址，对近似替换地址告警。
- **出站 exfil 链检测**——在行为序列检测之上识别多步密钥外泄模式（仅通知、保守默认）。
- **canary 诱饵检测**——在敏感凭据/钱包目录布放诱饵文件，工具调用一旦读到即触发入站告警。
- **配置化路由表**——协议兼容但路径非标准的中转站，只需加一行 `[[upstream.routes]]` 配置即可接入，零代码改动。
- **支持四家 agent**——Claude Code / OpenClaw / Hermes / Codex CLI。

---

## Quick Start

> ⚠️ Sieve 当前是**早期预览（0.1.0-alpha）**（见[项目状态](#项目状态)）。目前主用路径是源码自编译 / 邀请制 alpha 预览；下方命令描述正式发布形态，包管理器安装链路即将提供。

### 安装（Phase 1 仅 macOS）

大多数 `curl … | sh` 安装脚本要你盲信一段直接管进 shell 的脚本。Sieve 的安装器反着来：**在把任何东西落地前，它先用 cosign / sigstore（keyless 签名 + Rekor 透明日志）校验自己的 release 产物。** 二进制一旦被篡改、或来源不是 Sieve 的 release workflow，就**fail-closed 拒装**。一行命令，照样可验。校验是安装器替你做的家庭作业，不是甩给你的门槛。安全工具的安装器本就该长这样。
...
> 品牌短链 `sieveai.dev/install.sh` 将代理此脚本——即将提供。
...
- 检测 Claude Code / OpenClaw / Hermes / Codex CLI 装了哪些；
- 把 `ANTHROPIC_BASE_URL=http://127.0.0.1:11453` 写入 `~/.claude/settings.json`；
...
每次签名都会写入公开的 [Rekor](https://search.sigstore.dev/) 透明日志，每个 release 也可从源码逐字节复现——见 [deployment.md §3](./docs/guides/deployment.md)。任何「重签名」都会在 Rekor 留痕，无法静默替换。
...
# 可选：协议兼容但路径非标准的中转站，加一行路由即可接入，零代码改动。
[[upstream.routes]]
path = "/custom/chat"
provider = "openai"

[detection]
sequence_detection = false   # 行为序列检测，默认关闭
...
仓库现已 **public**，处于**早期预览（0.1.0-alpha）**阶段。源码公开是为兑现信任叙事——*可验证，而非仅信任*。目前主用路径是源码自编译 / 邀请制 alpha 预览，一键安装与规则自动更新体验正在打磨。

已具备的能力：双向检测（出站脱敏 + 入站 fail-closed 拦截 Critical 工具调用）、四路由内容类型对等（Anthropic/OpenAI × SSE/JSON）、Critical 在任何模式下不可关闭、检测完全本地零云校验、规则签名包下发、四家 agent 接入（Claude Code / OpenClaw / Hermes / Codex CLI）。检测引擎附带含真实攻击复现样本的完整测试套件。
