...
- **2026-05-05 单日**完成 unix-style 改造 v2.x 全部 5 项 TODO（13 commits）：multi-listener accept loop（[ADR-026](docs/design/ADR-026-port-based-listener-routing.md)）/ 协议错位 fail-closed / forwarder path prefix 修复 / 审计 provider_id 透传（SQLite v3）/ IPC `HealthResult.listeners` 数组 / SPEC-005 协议术语中性化（[ADR-028](docs/design/ADR-028-ipc-protocol-neutralization.md)）/ sieve-ipc crate 内部模块化（protocol/server/client 子目录）/ `sieve decisions` headless CLI / `sieve audit` unix-pipeable CLI / doctor multi-listener 体检。
- TODO-6 Network jail（[ADR-027](docs/design/ADR-027-network-jail-enforcement.md)）推后到 v3.x post-GA opt-in。
- **2026-05-03 基础**：v2.0 + v2.1 代码 + SPEC-005 v2 协议改造 + v2.0+ 兼容扩展（list_rules / purge_history）；Cargo workspace 6 crate；P0/P1/P2 + 善后全清；e2e 集成测试 6 场景。
...
2. **绝不联网做 verifier** —— 任何外部 token / 签名 / 规则的远端校验都摧毁产品定位。**唯一允许的出站**：①上游 LLM API（用户主动调）②`updates.sieve.app` manifest（每天 4 次,带 `?v=&os=&arch=&uid=&ch=` 5 字段,可关）③`cdn.sieve.app` 规则正文。详见 [ADR-003 amended](docs/design/ADR-003-local-only-no-cloud-verifier.md) + [ADR-030](docs/design/ADR-030-update-telemetry-channel.md)
