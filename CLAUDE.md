...
2. **绝不联网做 verifier** —— 任何外部 token / 签名 / 规则的远端校验都摧毁产品定位。**唯一允许的出站**：①上游 LLM API（用户主动调）②`updates.sieveai.dev` manifest（每天 4 次,带 `?v=&os=&arch=&uid=&ch=` 5 字段,可关）③`cdn.sieveai.dev` 规则正文。详见 [ADR-003 amended](docs/design/ADR-003-local-only-no-cloud-verifier.md)（更新通道协议详见内部记录）
