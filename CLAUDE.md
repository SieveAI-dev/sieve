...
**Pre-Code 设计阶段**——尚未写一行 Rust 代码，仓库内只有文档（`docs/` + `tasks/` + `README.md` + `.cursorrules`）。任何"读源码"的探索都会落空，先去读文档。Cargo workspace 在 Week 1 启动后才会出现。
...
2. **ADR** — [docs/design/ADR-INDEX.md](docs/design/ADR-INDEX.md)（7 个已接受 + 候选清单 ADR-008/009/010）
...
10. **Day 1 GitHub repo 公开 README + 架构文档** —— 代码 GA 后开源（MIT）
...
5. GitHub repo 公开（README + 架构文档可见，代码仍私有到 Week 12 GA）

---

## 常用命令（Pre-Code，仅占位）

当前阶段无 build / test / lint 命令——Cargo workspace 尚未初始化。Week 1 启动后将对应：
...
cargo test -p sieve-core sse_parser   # 跑单个测试

# Fuzz（PR 不带 fuzz 不合并，针对 SSE / 规则 / 工具调用判定）
cargo +nightly fuzz run sse_parser

# Benchmark（验证 P99 < 20ms，PRD §6.4）
cargo bench -p sieve-core
```

实际命令以 Week 1 落地后的 Cargo workspace 为准。本节在代码进仓时同步更新。
