...
- **2026-05-05 单日**完成 unix-style 改造 v2.x 全部 5 项 TODO（13 commits）：multi-listener accept loop（[ADR-026](docs/design/ADR-026-port-based-listener-routing.md)）/ 协议错位 fail-closed / forwarder path prefix 修复 / 审计 provider_id 透传（SQLite v3）/ IPC `HealthResult.listeners` 数组 / SPEC-005 协议术语中性化（[ADR-028](docs/design/ADR-028-ipc-protocol-neutralization.md)）/ sieve-ipc crate 内部模块化（protocol/server/client 子目录）/ `sieve decisions` headless CLI / `sieve audit` unix-pipeable CLI / doctor multi-listener 体检。ADR-030 sieve-updater crate 客户端代码 + SPEC-006 manifest 协议规格 + 六处 docs 同步全部落地（TODO-7~12 + TODO-17/18）。
- TODO-6 Network jail（[ADR-027](docs/design/ADR-027-network-jail-enforcement.md)）推后到 v3.x post-GA opt-in。
- **2026-05-03 基础**：v2.0 + v2.1 代码 + SPEC-005 v2 协议改造 + v2.0+ 兼容扩展（list_rules / purge_history）；Cargo workspace 六个 crate（sieve-updater 于 2026-05-05 ADR-030 落地时新增为七个）；P0/P1/P2 + 善后全清；e2e 集成测试 6 场景。
- **质量基线**：workspace 760 passed / 7 ignored / 0 failed；fmt / clippy / deny 全绿（2026-06-07 修复 13 个长期红测试后核实；此前 "725/0 failed" 为旧基线、PROGRESS 的 "760 passed" 曾把总数误作通过数，详见 tasks/PROGRESS.md 2026-06-07 条目）。
- **GUI 仓 follow-up**：sieve-gui-macos 仓 Swift 代码读 `health.listeners` 数组（向后兼容期内 `listen` 单字段仍发，daemon 可独立 ship 不阻塞）。

**当前进度真实源**：[`tasks/PROGRESS.md`](tasks/PROGRESS.md) —— 任何任务前先看，完成后必更新。`tasks/` 顶层只保留 `PROGRESS.md` / `roadmap.md` / `lessons.md` / `_archive/`；`docs/review/` 同理保留近期 + `_archive/`。详见全局 CLAUDE.md "`tasks/` 目录规范"。
...
1. **PRD v2.0** — [docs/prd/sieve-prd-v2.0.md](docs/prd/sieve-prd-v2.0.md)（已锁定执行，唯一权威源）
2. **ADR** — [docs/design/ADR-INDEX.md](docs/design/ADR-INDEX.md)（25 个已接受 + 候选清单 ADR-008/009/010）
...
2. **绝不联网做 verifier** —— 任何外部 token / 签名 / 规则的远端校验都摧毁产品定位。**唯一允许的出站**：①上游 LLM API（用户主动调）②`updates.sieveai.dev` manifest（每天 4 次,带 `?v=&os=&arch=&uid=&ch=` 5 字段,可关）③`cdn.sieveai.dev` 规则正文。详见 [ADR-003 amended](docs/design/ADR-003-local-only-no-cloud-verifier.md) + [ADR-030](docs/design/ADR-030-update-telemetry-channel.md)
...
14. **用户规则系统 fail-safe**（v2.0）—— 用户规则文件加载失败 / pattern 编译失败 / 安全 lint 拒绝 → daemon 必须正常启动 + 系统规则全功能；用户规则不能 override 或 suppress 系统 Critical；用户规则只能 High Ask/Warn/Mark，不能 Block / HookTerminal。详见 ADR-020 + ADR-024 LayeredEngine 合并顺序
15. **行为序列检测保守起步 + beta 默认关闭**（v2.0）—— IN-SEQ-* 仅触发 StatusBar 通知，不引入新 Block 路径；GA 默认关闭（feature flag `sequence_detection = false`），闭测用户主动 opt-in；GA 营销不承诺行为序列。升级为 Block 类需 4 周 ≥ 50 样本 + FP < 0.5% + 新 ADR。详见 ADR-022
16. **所有入站能力必须经过 content-type 路由矩阵测试**（v2.0，v1.5.4 P0 教训永久化）—— 任何新增入站功能必须有集成测试覆盖 4 类组合（Anthropic SSE / Anthropic JSON / OpenAI SSE / OpenAI stream=false JSON）；新功能只挂 SSE 不挂 JSON 视为 P0 漏洞。详见 ADR-025
...
| `sieve-updater` | manifest 协议客户端 / install-id 生成 / 6h 定时器 / ed25519 + sha256 校验 / 三个 env var 解析（ADR-030） | 不参与请求处理 / 不依赖 sieve-core 业务逻辑 / 不做规则文件原子替换（属于 sieve-rules） |
...
| 工程硬约束变化（PRD §9 十六条） | PRD-sieve 版本演进表 + ADR + .cursorrules §二 + CHANGELOG `[BREAKING]` | P0 |
...
候选 ADR：ADR-008（426 状态码）、ADR-009（Windows 服务）、ADR-010（加密支付通道）。已接受：ADR-012（Native GUI App）、ADR-013（IPC 协议）、ADR-014（双层防御）、ADR-015（sieve setup 工具）、ADR-016（处置矩阵二维化）、ADR-018（OpenAI 协议适配）、ADR-019（X-Sieve-Origin header）、ADR-020（用户规则系统）、ADR-021（三态决策 + 灰名单）、ADR-022（行为序列窗口）、ADR-023（进程上下文审计）、ADR-024（规则引擎抽象）、ADR-025（content-type 路由矩阵）。
...
- **[redacted] + Stripe + [redacted]**，[redacted]（[ADR-005](docs/design/ADR-005-overseas-legal-entity.md)）;**Phase 1 Stripe / 加密支付通道实际启用推后到 Phase 2**（[ADR-029](docs/design/ADR-029-free-first-defer-monetization.md)）
- **[redacted] to-C 公开商业化营销**——Twitter / Hacker News / Mirror 是主战场，微信 / 小红书 / 知乎 / B 站不规划（[PRD §11.5.2 渠道分级](docs/prd/sieve-prd-v2.0.md#1152-营销渠道分级)）
- **Phase 1（GA 后 6–12 个月）零收入,装机量为唯一指标**（[ADR-029](docs/design/ADR-029-free-first-defer-monetization.md)）;原 PRD §7「试用期 + [redacted]/月」定价模式被 ADR-029 替换为纯免费;装机量数据来源 = manifest 请求统计（[ADR-030](docs/design/ADR-030-update-telemetry-channel.md)）
- **永久排除「中转站认证 + 排名广告」收费模式**（ADR-029 §决策 2 评级机构利益冲突）
- ~~18 个月 [redacted] ≥ [redacted]~~ → **修订为「[redacted]优先,[redacted]推后到 Phase 2 装机数据出来后再定」**——**不追独角兽，不融资，不招人**
