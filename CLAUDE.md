...
**v2.0 + v2.1 代码已落地，正在 SPEC-005 v2 协议代码改造期**（gap 分析 P0 × 5 / P1 × 10 / P2 × 6，2026-05-02 r5 冻结）。Cargo workspace 6 crate（`sieve-core` / `sieve-rules` / `sieve-policy` / `sieve-cli` / `sieve-ipc` / `sieve-hook`）+ fuzz crate；sigstore CI pipeline 在跑。源码在 `crates/` 下。

**当前进度真实源**：[`tasks/PROGRESS.md`](tasks/PROGRESS.md) —— 任何任务前先看，完成后必更新（移到「已完成」段并把下一项拉进「进行中」）。`tasks/` 顶层只保留 `PROGRESS.md` / `roadmap.md` / `lessons.md` / `_archive/`；`docs/review/` 同理保留近期 + `_archive/`。详见全局 CLAUDE.md "`tasks/` 目录规范"。
