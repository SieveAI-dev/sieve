...
- **2026-05-05 单日**完成 unix-style 改造 v2.x 全部 5 项 TODO（13 commits）：multi-listener accept loop（[ADR-026](docs/design/ADR-026-port-based-listener-routing.md)）/ 协议错位 fail-closed / forwarder path prefix 修复 / 审计 provider_id 透传（SQLite v3）/ IPC `HealthResult.listeners` 数组 / SPEC-005 协议术语中性化（[ADR-028](docs/design/ADR-028-ipc-protocol-neutralization.md)）/ sieve-ipc crate 内部模块化（protocol/server/client 子目录）/ `sieve decisions` headless CLI / `sieve audit` unix-pipeable CLI / doctor multi-listener 体检。sieve-updater crate 客户端代码 + SPEC-006 manifest 协议规格 + 六处 docs 同步全部落地（TODO-7~12 + TODO-17/18）。
...
2. **ADR** — [docs/design/ADR-INDEX.md](docs/design/ADR-INDEX.md)（28 个已接受，含 ADR-033 上游代理 / ADR-034 GA 密钥 gate；候选清单 ADR-008/009）
...
2. **绝不联网做 verifier** —— 任何外部 token / 签名 / 规则的远端校验都摧毁产品定位。**唯一允许的出站**：①上游 LLM API（用户主动调）②`updates.sieveai.dev` manifest（每天 4 次,带 `?v=&os=&arch=&uid=&ch=` 5 字段,可关）③`cdn.sieveai.dev` 规则正文。详见 [ADR-003 amended](docs/design/ADR-003-local-only-no-cloud-verifier.md) 与 [ADR-030](docs/design/ADR-030-update-telemetry-channel.md)（更新通道协议）
...
15. **行为序列检测保守起步 + beta 默认关闭**（v2.0）—— IN-SEQ-* 仅触发 StatusBar 通知，不引入新 Block 路径；GA 默认关闭（feature flag `sequence_detection = false`），闭测用户主动 opt-in；GA 不对外承诺行为序列。升级为 Block 类需 4 周 ≥ 50 样本 + FP < 0.5% + 新 ADR。详见 ADR-022
...
候选 ADR：ADR-008（426 状态码）、ADR-009（Windows 服务）。已接受：ADR-012（Native GUI App）、ADR-013（IPC 协议）、ADR-014（双层防御）、ADR-015（sieve setup 工具）、ADR-016（处置矩阵二维化）、ADR-018（OpenAI 协议适配）、ADR-019（X-Sieve-Origin header）、ADR-020（用户规则系统）、ADR-021（三态决策 + 灰名单）、ADR-022（行为序列窗口）、ADR-023（进程上下文审计）、ADR-024（规则引擎抽象）、ADR-025（content-type 路由矩阵）。
