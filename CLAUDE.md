...
- **海外公司主体 + Stripe + 加密支付双通道**，不接受国内任何收款主体（[ADR-005](docs/design/ADR-005-overseas-legal-entity.md)）;**Phase 1 Stripe / 加密支付通道实际启用推后到 Phase 2**（[ADR-029](docs/design/ADR-029-free-first-defer-monetization.md)）
- **境内不做 to-C 公开商业化营销**——Twitter / Hacker News / Mirror 是主战场，微信 / 小红书 / 知乎 / B 站不规划（[PRD §11.5.2 渠道分级](docs/prd/sieve-prd-v2.0.md#1152-营销渠道分级)）
- **Phase 1（GA 后 6–12 个月）零收入,装机量为唯一指标**（[ADR-029](docs/design/ADR-029-free-first-defer-monetization.md)）;原 PRD §7「14 天试用 + $49/月」定价模式被 ADR-029 替换为纯免费;装机量数据来源 = manifest 请求统计（[ADR-030](docs/design/ADR-030-update-telemetry-channel.md)）
- **永久排除「中转站认证 + 排名广告」收费模式**（ADR-029 §决策 2 评级机构利益冲突）
- ~~18 个月 MRR 目标 ≥ $25K~~ → **修订为「装机量阈值优先,MRR 目标推后到 Phase 2 装机数据出来后再定」**——**不追独角兽，不融资，不招人**
