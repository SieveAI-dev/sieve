...
| `applies_to` | String[] | yes | — | no | 暂停影响的 `disposition` 列表（取自 §5.3 enum 值）；**永远不包含 `gui_popup` / `hook_terminal` 中关联 critical_lock 规则的项**（PRD §9 #3 / #8；`gui_popup` wire 值含义见 §5.3 标注） |
