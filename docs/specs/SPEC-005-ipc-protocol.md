# SPEC-005: Sieve daemon ↔ GUI IPC 协议

> Version: v2.0 — 2026-05-02
> Status: **Frozen**（经 5 轮 codex review 全部 CLOSED，可进入代码改造阶段）
> 关联 ADR：ADR-013（IPC 协议）、ADR-014（双层防御）、ADR-016（处置矩阵二维化）、ADR-019（X-Sieve-Origin header）、ADR-021（三态决策 + 灰名单）
> 关联 PRD：v2.0 §5.4 / §5.7 / §6.5、sieve-gui-macos PRD v1.0
...
**SPEC-005 是 daemon (`sieve` 仓库) 与 macOS GUI (`sieve-gui-macos` 仓库) 之间 IPC 协议的唯一权威源。**

- 任何方法名、字段名、枚举值、错误码、行为约定的变更，**必须先改本文件**，再分别在两个仓库落代码
- daemon 仓库 [`docs/api/api-reference.md §6`](../api/api-reference.md) 收敛为本 SPEC 的索引，不再保留 schema 表
- GUI 仓库 `docs/api/ipc-protocol.md` 收敛为"GUI 实现注解"，仅描述 GUI 端解析容错、UI 状态机映射、超时降级等本地行为
...
- HIPS 弹窗 UI 行为（见 [SPEC-002](SPEC-002-hips-popup-behavior.md)）
...
GUI 连接前必须校验 socket 文件权限与所有者；不符合时拒连并提示用户检查 `~/.sieve/` 安装状态。
...
| `context_hint` 字段 | 200 个 Unicode scalar（Rust `str::chars().count()` / Swift `String.unicodeScalars.count`） | GUI **MUST** 在提交层校验并 **拒绝**用户超限提交（不静默截断）；daemon 收到超限值 **MUST** 返 `-32602 invalid_params`，**不**做隐式截断 |
...
- **JSON-RPC 2.0** 双向：daemon 与 GUI 双方都可发起 request（含 `id`）和 notification（无 `id`）
- daemon 同一时间可有多个 GUI 连接（多窗口 / 多用户场景），所有广播类 notification 走 fan-out
- `request_decision` 仅发送给 `gui_writers[0]`（最早连接的 GUI）；当该 GUI 答复或断开后再切到下一个
...
| 取代 | `v1`（GUI 文档 v1.0 + daemon v1.5 实现，schema 错位严重） |

### 2.2 协商时序

1. GUI 连接 socket
2. daemon **立即** 主动发 `sieve.hello` notification（见 §3）
3. GUI 检查 `params.protocol_version`：
   - 在 GUI 白名单内（当前白名单 `["v2"]`）→ 进入 connected 状态
   - 不在白名单 → 关闭连接，UI 引导升级；GUI 端不向 daemon 回任何消息
4. daemon 不识别 GUI 后续请求中的方法名 → 返 `-32601 method_not_found`（保留向前兼容性）

### 2.3 版本号语义

- `vN`（无小版本）— **major 版本，强不兼容**：删字段、改字段语义、改方法名、改枚举集合、改握手时序
- 向前兼容的字段新增（GUI 必须忽略未知字段）—— 不递增 `protocol_version`；在 SPEC 中以 "Since v2.x" 标注
- 错误码新增（GUI 必须有 fallback "未知错误" 文案）—— 不递增
...
- **重发**：daemon 不主动重发 hello；GUI 重连建立新 socket 时再发一次
...
| `daemon_boot_id` | `Uuid` | yes | — | no | 每次 daemon 进程启动时新生成；GUI 用此值判断是否经历过 daemon 重启（见 §3.4） |

### 3.3 GUI 端期望行为

1. 校验 `protocol_version`，不匹配则关闭连接（不允许字段嗅探兼容）；同时 GUI **MUST** 进入"协议版本不匹配"UI 状态：
   - 显示模态或显著横幅，文案 **MUST** 至少包含两个事实：(a) "Sieve daemon 与 GUI 协议版本不兼容"；(b) "需要升级 GUI 至 v1.0+ 或将 daemon 降级到兼容版本"
   - 文案 **MUST** 提供至少一个可点击操作：打开 GitHub Releases 链接 / 打开"关于"窗口 / 打开"设置 → 检查更新"入口（具体三选一由 GUI 实现决定）
   - 此状态下 **MUST** 禁用所有 GUI → daemon 写入操作（暂停 / preset / 灰名单等），仅允许只读浏览本地缓存
2. 缓存 `daemon_version` 到本地用户设置（GUI 重启后展示"上次连接到 daemon vX.Y.Z"）
...
GUI 重连（含 daemon 重启 / 网络中断 / 主动断开重连等所有路径）建立新 socket 后，**MUST**：

1. **丢弃**所有跨上次连接的 `request_decision` inflight 状态（pendingQueue + activeRequest）。理由：daemon 重启后旧 oneshot channel 已失效；daemon 会重新计算所有 inflight 检测的 `default_on_timeout` 兜底。即便是同一个 daemon 进程（仅网络中断后重连），跨连接的 `request_id` 也已经过 fail-closed 处置，GUI 端再持有旧 id 无意义
2. **不**重发上次连接 inflight 的 `decision_response`。daemon 不会接受跨连接的 response，且重发会污染 audit
3. **UI 文案选择**：根据 `sieve.hello.daemon_boot_id` 与本地缓存的 `last_seen_daemon_boot_id` 比较：
   - **首次连接**（GUI 进程内此前从未有过 connected state，且本地无 cached boot_id）→ **不弹任何 toast**，仅初始化 `last_seen_daemon_boot_id` 后进入正常 connected 流程
...
> - 重启时 oneshot channel 自然失效；GUI 重连后第一条消息**必须**是新 `sieve.hello`
...
- **GUI 端超时**：30 秒内（含 heartbeat 与任何业务消息）未收到任何来自 daemon 的字节 → 视为失联，关闭并按指数退避重连
...
`params` 字段缺省（不发空对象），GUI 解析时不读取 params。
...
| `JsonRpcId` | String 或 Int（JSON-RPC 2.0 允许两种） | 本协议中所有 request 的 `id` **MUST** 使用 `Uuid` 格式 String，便于审计串联；GUI 端响应时 echo 原 `id` 字面值 |
...
> Critical 规则的 `default_on_timeout` 由 daemon 强制为 `"block"`，GUI 不能通过任何方法（含 `set_preset_overrides`）覆盖。

### 5.6 `preset_mode`

`"strict"` | `"standard"` | `"relaxed"` | `"custom"`

> v1 旧值 `"default"` 在 v2 中重命名为 `"standard"`，daemon 与 GUI 两端代码必须同步替换。
...
### 5.10 `ui_phase`（`decision_response.ui_phase_when_clicked`，仅 GUI 答复时附带）

`"blue"` | `"orange"` | `"red"` —— 对应 SPEC-002 的弹窗三段倒计时颜色，便于 daemon 审计用户在哪一阶段做的决定。
...
**SPEC-005 只定义 wire 字段**——即 GUI 与 daemon 之间通过 Unix socket JSON-RPC 实际传输的 JSON 字段。daemon 内部的 Rust 数据结构（`crates/sieve-ipc/src/protocol.rs::DecisionRequest`、`crates/sieve-core/...` 中的命中聚合中间表示等）**不**受本 SPEC 直接约束，可以自由演化。
...
- socket JSON-RPC 写入（GUI 路径）→ 经过 wire DTO 适配层，按本 SPEC §6.1 序列化

**禁止**：把本 SPEC 的字段直接映射到 hook pending file，也**禁止**把 hook pending file 的字段透传到 GUI socket。两条路径的演化解耦。

### 6.1 `sieve.request_decision`（daemon → GUI request）
...
| `rule_id` | String | yes | — | no | 命中规则 ID，如 `IN-CR-05` |
| `title` | String | yes | — | no | 已本地化（语言由 daemon 决定）的简短标题 |
| `severity` | enum (§5.1) | yes | — | no | |
| `direction` | enum (§5.2) | yes | — | no | |
| `disposition` | enum (§5.3) | yes | — | no | 决定 GUI 渲染样式 |
| `timeout_seconds` | u32 | yes | — | no | 范围 30–120 |
| `default_on_timeout` | enum (§5.5) | yes | — | no | Critical 强制 `"block"` |
| `allow_remember` | bool | yes | — | no | daemon 端通过 `is_critical_locked(rule_id)` 计算；内置 Critical 规则强制 `false`，GUI 不得覆盖此值。详见下方"Allow Remember 四道防线"一节 |
| `merged` | bool | no | `false` | no | `true` 时必须同时给 `issues[]` |
| `received_at_daemon` | `Timestamp` | yes | — | no | 用于 GUI 计算超时进度条 |
| `context` | Object | no | `null` | yes | 见 §6.1.3；`null` 或缺失时 GUI 走通用渲染 |
...
**Allow Remember 四道防线**（PRD §5.4 在协议层的体现）：

| # | 防线 | 责任方 | 协议要求 |
|---|---|---|---|
| 1 | daemon 计算 | daemon | daemon **MUST** 在发送请求前调用 `is_critical_locked(rule_id)`，命中即 `allow_remember=false`；**禁止**让用户配置 override 此字段 |
| 2 | GUI UI 禁用 | GUI | `allow_remember=false` 时 GUI **MUST** 在弹窗 UI 中禁用且灰显 Remember 控件，**禁止**渲染为可交互状态（不允许"灰显但仍可点击"，不允许键盘快捷键绕过） |
| 3 | GUI 编码层强制 | GUI | GUI 提交 `decision_response` 前 **MUST** 在编码层强制：当对应请求的 `allow_remember=false` 时，序列化时把 `remember` 字段强制写为 `false`（即便上层错误地传入 `true`） |
...
任意一道防线失效都不影响整体安全：1+4 daemon 双向校验保证 GUI 端被绕过时仍安全；2+3 GUI 双重保护防止 UI bug 让用户误操作。代码 PR 必须为每道防线写独立测试。
...
| `rule_id` / `context` / `recommendation` | **不发**（避免 GUI 误用合并请求里的单 issue 字段） |
| `issues` | 数组，每项见下表 |
...
| `generic_json` | 兜底 | `payload: any` —— 任意 JSON 子树，GUI 走通用 key-value 渲染 |

**GUI 不识别的 `template` 值必须降级为 `generic_json` 处理**，并把整个 context 对象作为 `payload` 渲染。
...
**GUI 默认按钮规则**：当 `recommendation` 缺失或 `recommendation.confidence != "high"` 时，HIPS 主按钮（含键盘 Return 默认）必须为 **拒绝**。详见 [SPEC-002](SPEC-002-hips-popup-behavior.md)。

### 6.2 `sieve.decision_response`（GUI → daemon response）

GUI 通过 JSON-RPC response 形式回复 `request_decision`，**复用同一 `id`**，无独立 method 字段。
...
| `decision` | enum (§5.4 子集) | yes | — | no | `"allow"` / `"deny"`（GUI 端不发 `redact_and_allow`） |
| `decided_at` | `Timestamp` | yes | — | no | GUI 端时钟，daemon 用于审计 |
| `by_user` | bool | yes | — | no | `true` = 用户主动操作；`false` = GUI 端兜底超时（理论上不应出现，daemon 端有 oneshot 超时；保留以防 GUI 进程卡死时仍上报） |
| `remember` | bool | no | `false` | no | GUI 用户勾选"永久允许"；**当 request 中 `allow_remember=false` 时 GUI 必须强制 `false`**（编码层 reject，UI bug 兜底） |
| `context_hint` | String | no | `null` | yes | 用户备注，≤ 200 个 Unicode scalar（见 §1.3）；GUI MUST 拒绝超限提交 |
| `ui_phase_when_clicked` | enum (§5.10) | no | `null` | yes | GUI 弹窗不展示倒计时阶段时（如 disposition≠gui_popup）允许为 `null` |
...
GUI 在用户主动取消、GUI 渲染失败、GUI 进程关停等场景下用 JSON-RPC error 回复，复用同 `id`：
...
错误码列表见 §12。daemon 收到任何 GUI 错误 → 按 `default_on_timeout` 处置该请求并写 audit `kind=decision_response_error`。

### 6.3 `sieve.request_decision_canceled`（daemon → GUI fan-out notification）
...
- 该 `request_id` 是 activeRequest → 关闭弹窗，菜单栏恢复 normal
...
- **fan-out**：广播给所有连接的 GUI 客户端；写入失败的 sender 在下次广播时 lazy 清理
...
| `detail` | String | no | `null` | yes | 长文案，GUI tooltip 或副标题展示 |
...
|---|---|---|
| `sequence_hit` | 行为序列窗口（IN-SEQ-*）匹配到 kill chain | `0`（用户需手动确认） |
| `outbound_redacted` | OUT-01~05/12 AutoRedact 命中 | `5` |
| `hook_terminal` | sieve-hook 在 TTY 完成的事件回报，让 GUI 同步显示一份"刚才在终端发生了什么" | `8` |
...
## 9. GUI 控制面方法（GUI → daemon request/response）
...
| `applies_to` | String[] | yes | — | no | 暂停影响的 `disposition` 列表（取自 §5.3 enum 值）；**永远不包含 `gui_popup` / `hook_terminal` 中关联 critical_lock 规则的项**（PRD §9 #3 / #8） |

#### Fan-out 强制

按 §10.0.1，daemon **MUST** 在返回 result 之前先 fan-out 一条 `sieve.paused_changed { source: "gui", origin_request_id: <本请求 id> }`，所有当前连接的 GUI 都会收到。`minutes=0`（恢复）时也必须 fan-out（`paused=false` + `paused_until=null` + `reason="user_request"`）。
...
成功后 daemon 必须 fan-out 一条 `sieve.preset_changed` 通知（见 §10.1），`source: "gui"`，并附 `origin_request_id` 等于本次请求的 `id`，便于发起方 GUI 识别回声。
...
| `listen` | ListenSnapshot | yes | — | no | 见 §9.5.4 |
...
#### 9.5.4 `ListenSnapshot` / `GraylistSnapshot` / `IpcSnapshot`

```jsonc
{ "addr": "127.0.0.1", "port": 11453 }       // ListenSnapshot
...
| IpcSnapshot | `connected_clients` | u32 | yes | — | no | 当前 IPC socket 已 accept 的 GUI 客户端数 |
| IpcSnapshot | `total_decisions_inflight` | u32 | yes | — | no | 当前等待 GUI 响应的 `request_decision` 数 |
...
## 10. 暂停与 preset 状态广播（daemon → GUI fan-out notification）

### 10.0 多 GUI 回声防护（适用于本章所有通知）

为支持多 GUI 并存场景下的回声去重，本章所有 fan-out notification **MUST** 携带 `origin_request_id` 字段：

| 字段 | 类型 | required | default if absent | null accepted | 说明 |
|---|---|---|---|---|---|
| `origin_request_id` | `Uuid` | yes | — | yes | 当通知由某条 GUI request 触发时，**MUST** 设为该 request 的 `id` 字面量；当通知由 CLI / 配置 reload / 自动恢复 / daemon 启动等非 GUI 路径触发时，**MUST** 设为 `null` |

#### 10.0.1 daemon 端发送顺序与串行化（规范性）

为避免 GUI 端竞态，daemon **MUST** 严格按以下顺序处理任何由 GUI request 触发的状态变更（含 §9.1 / §9.2 / §9.3 等会触发 fan-out 通知的 mutating 控制面方法）：

1. 应用变更（修改内部状态）
2. **先** fan-out 对应通知（含 `origin_request_id = <本 request id>`），**等待**写入所有当前连接的 GUI socket（write buffer flushed 即可，不等 OS-level ack）
3. **再** 返回 `result` response 给发起方 GUI
...
**全局串行化要求**：daemon **MUST** 通过单一状态变更队列 / mutex 串行化所有 mutating 控制面请求（`set_paused` / `set_preset` / `set_preset_overrides` / `reload_config` 等）；**禁止**多请求间交错执行 apply / fan-out / result 步骤。多 GUI 同时发起变更时按 daemon 接收顺序线性化，**最后一个生效者获胜**（last-write-wins）。

**只读请求**（`sieve.health` / `sieve.list_graylist` / `sieve.evaluate` 等）**不**经过此队列，可以与 mutating 请求并发处理，避免 read-only 请求被卡住。

**慢 GUI 防御**：fan-out 写入慢 GUI 的 socket buffer 时 **MUST** 设 bounded write timeout（推荐 2 秒）；超时**或**任何 write error（EPIPE / ECONNRESET / EBADF 等）**MUST** 视为该 GUI 失联，断开连接 + 清理其 sender，**不**阻塞当前请求的 result 路径。

> **理由**：发起方 GUI 收到 result 时，必然已经收到对应的 fan-out 通知，inflight id 集合中该 id 还在 → fan-out 被识别为回声忽略；result 处理路径再做一次本地状态更新。这样状态机不会出现"先收到 result 把 inflight id 移除，再收到 fan-out 误以为是外部变更而重复刷新"的竞态。
>
> 多 GUI 场景下其他 GUI 也按此顺序收到通知（origin_request_id 不在它们的 inflight 集合中），会正确同步状态。

#### 10.0.2 GUI 端 inflight id 生命周期（规范性）

> **作用域**：本节定义的 inflight id 集合**仅用于** §9.1–§9.3 等会触发 fan-out 的 GUI-originated mutating 控制面请求的回声去重。它**不是** `sieve.request_decision` 的 120 秒业务超时机制（后者完全在 daemon 侧由 oneshot channel + `default_on_timeout` 管理）。两者不要混淆。

GUI 端维护 `inflight_mutating_request_ids: Set<Uuid>`：
...
**实现简化方案**：GUI 直接在 result handler 末尾移除 inflight id；按 §10.0.1 daemon 顺序保证，到这一步时同 id 的 fan-out 已经被路由处理完。

> **为什么不用 `source`**：v1 设计的 `source: "gui"` 只能区分通知类型，无法区分"哪个 GUI"；多 GUI 场景下 A 改 preset，B 也会按 `source==gui` 判定为回声并忽略，导致 B 状态不同步。`origin_request_id` 直接绑定到本 GUI 自己的 inflight id，对其他 GUI 来说该 id 自然不在集合内，会正确刷新。`source` 字段保留作为辅助信息（用于审计 / 日志）。
...
| `source` | enum | yes | — | no | `"cli"` / `"gui"` / `"config_reload"` —— 仅作为审计/日志辅助标签，不用于 GUI 回声判定 |
...
| 消息名 | 方向 | 类型 | 章节 |
|---|---|---|---|
| `sieve.hello` | daemon → GUI | notification | §3 |
| `sieve.heartbeat` | daemon → GUI | notification | §4 |
| `sieve.request_decision` | daemon → GUI | request | §6.1 |
| （`sieve.request_decision` 的 JSON-RPC response，无独立 method 字段） | GUI → daemon | response（同 id） | §6.2 |
| `sieve.request_decision_canceled` | daemon → GUI fan-out | notification | §6.3 |
| `sieve.notify_status_bar` | daemon → GUI fan-out | notification | §7 |
| `sieve.reload_user_rules` | CLI → daemon | notification | §8 |
| `sieve.set_paused` | GUI → daemon | request | §9.1 |
| `sieve.set_preset` | GUI → daemon | request | §9.2 |
| `sieve.set_preset_overrides` | GUI → daemon | request | §9.3 |
| `sieve.reload_config` | GUI → daemon | request | §9.4 |
| `sieve.health` | GUI → daemon | request | §9.5 |
| `sieve.evaluate` | GUI → daemon | request | §9.6 |
| `sieve.list_graylist` | GUI → daemon | request | §9.7 |
...
| `-32000 ~ -32099` | **daemon → GUI 业务错误**（保留段） |
| `-32100 ~ -32199` | **GUI → daemon 业务错误**（保留段） |
...
### 12.3 daemon → GUI 业务错误（`-32000 ~ -32099`）

| Code | 名称 | 触发场景 |
|---|---|---|
| `-32000` | `protocol_version_mismatch` | GUI 发来的请求中暗示的协议版本 daemon 不支持（保留；当前 daemon 通过 `sieve.hello` 主动协商，不主动用此码） |
...
| Code | 名称 | 触发场景 |
|---|---|---|
| `-32100` | `user_canceled_via_window_close` | 用户关闭弹窗未做决策 |
| `-32101` | `gui_render_failed` | GUI 端渲染异常（解析 request 失败、SwiftUI 抛错等） |
| `-32102` | `gui_shutdown_during_decision` | GUI 进程在决策过程中退出 |

> **v1 → v2 错误码迁移**：v1 中 `-32000 ~ -32002` 在 GUI → daemon 方向被占用为 `user_canceled_via_window_close` / `gui_render_failed` / `gui_shutdown_during_decision`，与 daemon → GUI 段位冲突。v2 起 GUI 错误码统一搬至 `-32100+`。
...
| `message` | String | yes | — | no | 与 §12 中"名称"列字面量相同（snake_case）；GUI 端可基于此选择本地化文案 |
...
1. **第一步：SPEC PR**（daemon 仓库）— 改 SPEC-005 + 关联 ADR（如涉及决策性变更）→ review → merge。此 PR 不含任何代码改动
2. **第二步：daemon 代码 PR** — 实现新 schema，更新 `crates/sieve-ipc/src/protocol.rs` + 测试 + fixture（见 §14）→ merge
3. **第三步：GUI 代码 PR** — 更新 Codable structs + IPCRouter + 单元测试，更新 `docs/external/upstream-references.md` 的 SPEC-005 commit pin → merge
4. 三个 PR 在描述中互相引用对方 commit hash + SPEC-005 commit hash

#### 13.3.2 发布顺序（关键，避免 GUI 先发布却连不上旧 daemon）

合 PR 顺序 ≠ 用户安装顺序。**真正避免破坏的是发布顺序**：

| 阶段 | daemon 发布 | GUI 发布 | 兼容窗口语义 |
|---|---|---|---|
| T0 | v1.x（旧协议 v1） | v0.x（旧协议 v1） | 用户在用 |
| T1 | **先发** v2.0（实现新协议 v2，**同时** 拒绝 v1 客户端 with `sieve.hello { protocol_version: "v2" }`） | 还是 v0.x | **GUI v0.x 连 daemon v2.0 → 立即握手失败 → UI 引导用户升级 GUI**。daemon 不向下兼容（v2 是 major bump） |
| T2 | v2.0 | **再发** v1.0（实现新协议 v2） | 双方 v2，恢复正常 |

**MUST**：
- daemon v2 发布前 **MUST** 在 release notes 里写明"需要 GUI v1.0+，旧 GUI 会进入 disconnected 状态"
- GUI v1.0 发布前 **MUST** 在 release notes 里写明"需要 daemon v2.0+，旧 daemon 会握手失败"
- daemon v2 发布后 **MUST** 至少有一个 GUI v1.0 RC 可供用户升级（避免用户升 daemon 后无 GUI 可用）

**不引入**多协议版本并存的兼容层。每次 major bump 都按上面流程，daemon 一刀切。理由：双 GUI / 双 daemon 协议矩阵在测试与 audit schema 上是噩梦，"快速强制升级"成本远小于"长期维护多版本"。
...
**采用 daemon 发布 release artifact，GUI 在代码 PR 中按 commit pin 拉取**——不使用 git submodule（避免 GUI 仓库每次 daemon commit 都触发提交）。
...
3. 打包 `tar -caf sieve-ipc-fixtures-v2.tar.zst manifest.json *.json`（**固定文件名**，不带 commit hash 后缀，避免 GUI CI 需要先查 commit 再拼 URL）
4. 同步生成 `sieve-ipc-fixtures-v2.tar.zst.sha256`
5. 上传到 GitHub Release：URL 模板 `https://github.com/doskey/sieve/releases/download/<release_tag>/sieve-ipc-fixtures-v2.tar.zst`（其中 `<release_tag>` 是 daemon 的 git tag，如 `v0.7.2`）
6. release notes 中列出 `spec_commit` / `daemon_commit` 关联（仅供人类阅读；GUI CI **不**依赖 release notes 内容做 discovery）

> **不做 commit-hash-suffix 命名**：旧方案 `sieve-ipc-fixtures-v2-<sha>.tar.zst` 要求 GUI CI 先查 daemon release 找包含某 spec_commit 的 release，scrape release notes 才能拿到精确 sha 后缀。改为固定命名 + release_tag 后，GUI CI 可直接用 URL 模板拼接，不依赖任何 release notes 内容。`spec_commit` 校验通过解压后的 `manifest.json` 完成。

#### 14.3.4 GUI 端同步

GUI 仓库根目录 `scripts/sync-ipc-fixtures.sh`：
1. 读 `docs/external/upstream-references.md` 中 SPEC-005 的 `<pinned commit>` + 关联的 `<daemon_release_tag>`（两个字段都 pin，禁止只 pin commit）
2. 直接构造 artifact URL：`https://github.com/doskey/sieve/releases/download/<daemon_release_tag>/sieve-ipc-fixtures-v2.tar.zst`（**固定 asset 命名**，不带 commit hash 后缀；同一 release 只有一份 fixture）
...
GUI 端代码 PR **MUST** 在同一 commit 中包含：(a) 更新 `upstream-references.md` 的 commit pin；(b) 跑 `sync-ipc-fixtures.sh` 后的 fixture 文件变更（含 manifest.json）。`Tests/SieveGUICoreTests/Fixtures/v2/` 目录由 git 跟踪，任何手改在 PR review 中都会暴露。
...
- 长期 TODO：把 fixture artifact 也纳入 sigstore 签名（与 daemon 二进制同流程，见 ADR-006），抵御 release 仓库泄露的高级威胁

#### 14.3.6 fixture 权威性

所有 fixture 的"权威版本"在 daemon 仓库 `crates/sieve-ipc/tests/fixtures/v2/`；GUI 仓库 `Tests/SieveGUICoreTests/Fixtures/v2/` 只消费、不生产。GUI 端发现 fixture 有问题 → 提 issue 到 daemon 仓库改源头，**禁止**只在 GUI 端改副本。
...
- API 总览：[../api/api-reference.md §6](../api/api-reference.md#6-cli-退出码--弹窗确认协议)（本 SPEC 取代）
- 架构：[../design/architecture.md](../design/architecture.md)
- ADR 索引：[../design/ADR-INDEX.md](../design/ADR-INDEX.md)
- ADR-013 IPC 协议：[../design/ADR-013-ipc-protocol.md](../design/ADR-013-ipc-protocol.md)
- ADR-021 三态决策与灰名单：[../design/ADR-021-tri-state-decision-and-graylist.md](../design/ADR-021-tri-state-decision-and-graylist.md)
- SPEC-001 sieve-hook 协议：[SPEC-001-sieve-hook-protocol.md](SPEC-001-sieve-hook-protocol.md)
- SPEC-002 HIPS 弹窗行为：[SPEC-002-hips-popup-behavior.md](SPEC-002-hips-popup-behavior.md)
- 数据模型：[../design/data-model.md](../design/data-model.md)
- GUI 仓库 ipc-protocol（实现注解）：`sieve-gui-macos/docs/api/ipc-protocol.md`
