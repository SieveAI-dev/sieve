...
更新通道在拉取规则的同一请求中附带匿名 install-id，而非开辟独立心跳通道：
...
- manifest 接口走自有服务器不挂 CDN（为保证可用性走自有服务器）
...
**防 CDN 被注入**：规则正文走 CDN，CDN 节点可能被攻击者注入恶意内容。ed25519 签名在客户端校验，即使 CDN 被完全攻陷也无法推送恶意规则。
...
| `SIEVE_NO_TELEMETRY` | 仍发更新请求但省略 `uid` 字段（仍能拿到规则更新，请求不包含安装标识符） | 未设 | 隐私敏感场景 |
...
- 服务端按日去重统计活跃安装数（`COUNT(DISTINCT uid)`）
...
> Sieve 每天 4 次连接更新服务器获取最新规则。请求会附带：Sieve 版本、操作系统、CPU 架构、一个本地随机生成的安装 ID（用于区分独立安装，不绑定您的账号或设备）。Sieve 不上传 prompt、response、API key 或任何使用记录。可在设置中关闭安装 ID 上报（规则更新不受影响），也可通过环境变量 `SIEVE_NO_UPDATE=1` 完全禁用更新检查。
