...
GUI 在 `false` 时**禁止渲染** Remember checkbox（不允许灰显代替；详见 sieve-gui-macos PRD §5.2.8 / §6.4 / §9 #1，独立仓库 `sieve-gui-macos/docs/prd/sieve-gui-macos-prd-v1.0.md`）。daemon 在 `decision_response.remember=true` 时做防线二二次校验，违规写 `kind=critical_lock_blocked`。
