...
| 检测项 | 方向 | 弹窗类型 | 触发时机 |
|-------|------|---------|---------|
| OUT-01~05, OUT-12, OUT-13, OUT-14 | 出站 | 无弹窗 | 自动脱敏，状态栏 5 秒提示 |
| OUT-06, OUT-08 | 出站 | GUI 弹窗 | 主代理出站 filter 命中后 hold 请求 |
| OUT-07, OUT-10 | 出站 | GUI 弹窗（强警告） | 主代理出站 filter 命中后 hold 请求 |
| OUT-09 | 出站 | GUI 弹窗（fail-closed） | 主代理出站 filter 命中后 hold 请求 |
| OUT-11 | 出站 | 无弹窗 | 状态栏标记 |
...
> **BIP39 编号与处置变更（v1.1）**：BIP39 助记词出站检测现编号 `OUT-14`、处置 `auto_redact`（checksum 通过的窗口改写为 `[REDACTED:OUT-14]` 后 `200` 转发，不弹窗、不阻断）。历史版本中该检测曾以 `OUT-09` 编号做 GuiPopup + 426 硬阻断；`OUT-09` 编号现属 Slack Token 规则——该规则处置仍为 `gui_popup` 且标记 fail-closed（不可关闭、不可永久白名单），以独立行保留在本弹窗矩阵中，与 BIP39 检测无关。
...
| 检测项 | 超时时长 | 超时默认 | 弹窗类型 |
|-------|---------|---------|---------|
| OUT-01~05, OUT-12, OUT-13, OUT-14 | 无超时 | 自动脱敏（不弹窗）| — |
| OUT-06, OUT-08 | 15 秒 | `allow`（脱敏后发送）| GUI 弹窗 |
| OUT-07, OUT-10 | 60 秒 | `deny`（完全拦截）| GUI 弹窗强警告 |
| OUT-09 | 60 秒 | `deny`（完全拦截）| GUI 弹窗（fail-closed）|
...
4. 校验位通过的高确定性私钥（OUT-07/OUT-10）例外——即使是出站也强弹窗，默认拒绝；OUT-09（Slack Token）为高置信度凭据格式，同样超时默认拒绝且 fail-closed；BIP39 助记词（OUT-14）虽同样过校验位，但按「出站脱敏不打断工作流」硬约束归自动脱敏类，不弹窗
...
critical_lock 保护的规则集合**不再硬编码 ID 名单**：由 `sieve-rules` 的运行时分类注册表在加载规则时从规则自身字段（`fail_closed` / severity）派生——所有 Critical 规则默认在内，另含显式标注 `fail_closed = true` 的高危规则。集合内规则无论 preset 如何配置，以下属性**不允许覆盖**：

```
fail-closed 注册表成员的强制约束：
  default_on_timeout 强制 = "deny"
  不允许 remember=true（永久白名单被拒）
  典型成员（GuiPopup 类）：IN-CR-01、IN-CR-05、OUT-07、OUT-09（显式 fail_closed）、OUT-10
```

`sieve-rules` crate 在加载 preset 配置时，对注册表内规则的 `default_on_timeout` 字段做 critical_lock 校验，验证失败直接 panic（daemon 启动失败，而非静默忽略）。
...
|-----------|---------------|------------|
| **Critical 锁集合**（fail-closed 运行时注册表成员，如 IN-CR-01 / IN-CR-05 / OUT-07 / OUT-09 / OUT-10）| **仍然弹窗**，不可暂停 | 与 §2.2 完全一致 |
...
| 规则归属 | `allow_remember` |
|---------|------------------|
| critical_lock fail-closed 运行时注册表内（从规则字段派生，如 IN-CR-01 / IN-CR-05 / OUT-07 / OUT-09 / OUT-10）| `false`（强制）|
| 其他系统规则 + 用户规则 | `true` |
