...
### ENV-MCP-\* / `.mcp.json` 命令注入扫描

`sieve doctor` 对本地 MCP 配置文件（当前目录 `.mcp.json` + `~/.claude.json`）做的静态卫生扫描，检出
「会被 shell 静默执行的 helper 命令字段」等启动时 RCE 入口（如 Claude Code `headersHelper` 字段：
非交互模式下信任确认被跳过，字段值直接 `exec()`）。`ENV-MCP-01`（Critical，静默 exec helper 字段）
命中让 doctor fail；`ENV-MCP-02`（High，项目本地 stdio `command` / schema 未知命令字段）仅告警。

与 *IN-MCP-\**（见 [MCP server](#mcp-server)）**命名空间不重叠且性质不同**：`IN-MCP-*` 是 Phase 2
针对**运行时 MCP 工具调用流量**的检测规则；`ENV-MCP-*` 是 Phase 1 针对**本地 MCP 配置文件**的静态
扫描（该攻击不经流量代理，故必须走本地体检）。
