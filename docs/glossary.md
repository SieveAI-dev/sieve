...
新用户从首次安装 Sieve 开始的完全功能使用期限，$0 计费。试用结束后，用户可选择 $49/月订阅或进入降级模式。详见 [PRD §7.1](../prd/sieve-prd-v1.3.md#71-单一定价)。
...
**P0 客群**：Crypto-native AI 重度开发者，日用 Claude Code ≥ 4 小时，持有 $10K+ crypto 资产，付费意愿强。**P1 客群**：智能合约开发者、bug bounty hunter、审计师，工作涉及单笔 $100K+ 价值的代码。详见 [PRD §3](../prd/sieve-prd-v1.3.md#3-用户画像)。
...
攻击方式之一（IN-CR-04）：诱导用户对恶意内容（如 drainer 合约的 Permit 调用）签名。Sieve 检测 verifyingContract 数字化绕过、已知 drainer 特征。
...
Sieve 的法律实体必须海外注册，不能是中国大陆个人或个体户。首选香港有限公司或新加坡 Pte Ltd。理由：Stripe 账户要求、中国大陆 crypto 合规监管、doskey 税务便宜。详见 [PRD §11.5.1](../prd/sieve-prd-v1.3.md#1151-公司主体与收款)。
