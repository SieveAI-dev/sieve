...
daemon 引入 `[[upstream]]` 数组的同时保证旧 sieve.toml 仍可用。验证：
...
- [ ] **Anthropic listener 收 OpenAI path → 400**：
  ...
  - daemon 日志含 `WARN ... protocol mismatch: path 隐含协议与 listener 声明冲突`
...
- [ ] 输出包含原 5 项 + 新增「multi-listener 全部端口可达（3 个 listener）」一项
- [ ] 全部 ✅
...
- [ ] **schema 版本**：
  ...
  返回 `3`（provider_id 列升级到 schema v3）。
...
## 9. sieve decisions headless CLI
