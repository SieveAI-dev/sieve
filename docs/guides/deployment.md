...
### 6a.5 [redacted]（v3.x，ADR-027 网络层硬隔离）
...
## 7e. 加密审计日志（`full` 档）部署指南（ADR-037）

> 适用对象：需要审计「Sieve 到底擦了什么、拦了什么」的实际（**脱敏后**）内容片段的用户——尤其闭测期排查规则误伤（FP）。完整决策见 [ADR-037](../design/ADR-037-encrypted-audit-log.md)，工程级设计见 [SPEC-009](../specs/SPEC-009-encrypted-audit-log.md)（Phase 2 落地）。
...
> **红线（不可违背）**：脱敏永远先于任何字节落盘。归档存的**永远是脱敏后内容**，脱敏前的明文密钥（API key / 助记词 / 私钥）无论是否加密都绝不落盘。详见 [ADR-037 决策 2](../design/ADR-037-encrypted-audit-log.md)。
...
> 这正是 [ADR-037 威胁模型](../design/ADR-037-encrypted-audit-log.md) 第 3 条「目录被误同步到云盘或被误 `git add`」要防的暴露面。归档加密能挡住「文件被读到」，但不该主动把它送出本机。
...
> 哈希链保证历史归档**不可悄悄改写 / 删除 / 重排**（呼应 [ADR-006](../design/ADR-006-sigstore-reproducible-build.md) 透明可验证叙事）。诚实标注的残余局限：它**挡不住「末尾追加伪造」**（持公钥的 daemon 可继续合法追加），保证的是历史不可被篡改。
