...
- 仅本地回环（`127.0.0.1`）—— `bind_addr` 任何非 `127.0.0.1` 值会触发 FATAL exit（PRD §9 #2）
...
### 6a.5 [redacted]（v3.x，网络层硬隔离）

network jail enforcement 计划在 v3.x post-GA opt-in 上线，届时多 listener 会与 macOS pf / Linux nftables uid-based egress filter 配合，实现"非 sieve 进程无法直连 LLM endpoint"。本节只覆盖 v2.x multi-listener，jail 部署见未来版本。
...
> 适用对象：需要审计「Sieve 到底擦了什么、拦了什么」的实际（**脱敏后**）内容片段的用户——尤其闭测期排查规则误伤（FP）。工程级设计见 [SPEC-009](../specs/SPEC-009-encrypted-audit-log.md)（Phase 2 落地）。
