...
## 4. SSE Fuzz 测试（[PRD §9 硬约束 #5](../prd/sieve-prd-v2.0.md#9-工程上必须做对的硬约束)）
...
[PRD §6.4](../prd/sieve-prd-v2.0.md#64-性能预算) 性能预算：
...
1. **Severity + FP 上限承诺** —— 引用 [PRD §6.5 误报率预算](../prd/sieve-prd-v2.0.md#65-误报率预算)；如想申报 Critical，须明确 `< 0.5%` 或更严格
...
2. **正则 / 词表匹配**
3. **校验位 / entropy / SHA-256**（如 [BIP39 SHA-256 校验](../prd/sieve-prd-v2.0.md#5-功能需求)，PRD §9 硬约束 #4）
4. **上下文关键词 + 占位符黑名单**
...
- 每周末汇总 FP 计数，超阈值（详见 [PRD §6.5](../prd/sieve-prd-v2.0.md#65-误报率预算)）必须当周修
...
- 改动摘要（**为什么** > 什么）
- §11.3 文档同步清单
- 是否触及 [PRD §9 硬约束](../prd/sieve-prd-v2.0.md#9-工程上必须做对的硬约束)（默认不允许绕过）
