...
> Version: v2.2 — 2026-07-10
>
> **状态：早期预览（0.1.0-alpha）。**
> 8 crate workspace，IPC + hook 路径跑通，行为序列检测（IN-SEQ-*）以 feature flag 形式 opt-in，用户规则 CLI 可用，benchmark CI gate 已接入 `.github/workflows/bench.yml`。
...
| 工具 | 版本 | 用途 |
|------|------|------|
| Rust | **1.88.0 stable**（已锁定在 `rust-toolchain.toml`,推荐 [`rustup`](https://rustup.rs/) 管理） | 主语言 |
...
├── crates/
│   ├── sieve-core/                 # 协议层 + SSE Parser + UnifiedMessage + Forwarder + ProviderCodec
│   ├── sieve-rules/                # 规则定义 + vectorscan 编译 + 匹配引擎 + Ed25519 验证
│   ├── sieve-cli/                  # 主二进制 + 配置 + 弹窗 + 审计 SQLite + 管理 API
│   ├── sieve-ipc/                  # IPC server：Unix socket + client 通知协议
│   ├── sieve-hook/                 # 极简 PreToolUse hook 二进制
│   ├── sieve-policy/               # 用户规则加载 + lint + LayeredEngine + 灰名单
│   ├── sieve-updater/              # 更新 manifest 客户端 + 规则包校验落盘
│   └── sieve-testing/              # 集成测试 harness
...
| crate | 职责 | 不允许做 |
|-------|------|---------|
| `sieve-core` | Pipeline / SSE Parser / UnifiedMessage / Forwarder / ProviderCodec | 任何 CLI / TUI / 配置加载 |
| `sieve-rules` | 规则定义 / vectorscan 编译 / 匹配引擎 / Ed25519 验证 | 任何网络 IO |
| `sieve-cli` | 入口 / 配置 / 弹窗 / 审计 SQLite / 管理 API | 直接做规则匹配（必须经 `sieve-rules`） |
| `sieve-ipc` | Unix socket IPC server + client 通知协议（pending/decision 目录扫描） | 任何规则匹配逻辑 |
| `sieve-hook` | 极简 PreToolUse hook 二进制（`#[cfg(target_os = "macos")]` 严格限定） | 直接访问 sieve-rules / sieve-core 内部实现 |
| `sieve-policy` | 用户规则加载 / lint / LayeredEngine 合并 / 灰名单管理 | 直接做匹配（调 sieve-rules）；suppress 系统 Critical |
| `sieve-updater` | 更新 manifest 客户端 / install-id / 校验落盘 | 参与请求处理 / 依赖 sieve-core 业务逻辑 |
| `sieve-testing` | 集成测试 harness | 进入发布二进制 |
...
```bash
rustup show                                    # 确认 toolchain == 1.88.0 stable（锁定在 rust-toolchain.toml）
cargo --version
clang --version
```
...
# clippy 分两步（CI 实态）：workspace 全量，再单独跑 sieve-cli 的可选特性
cargo clippy --workspace --all-targets --locked -- -D warnings          # 警告即错误
...
**数据集回归**（不阻塞 PR 但合并前必须人工跑过）：

```bash
# 跑数据集 baseline（标 #[ignore]，不在 cargo test 默认范围）
# 数据集分 attacks（按规则 ID + 用户最怕的场景分类）与 benign / benign-near
# （正常 Q&A + "看起来像攻击但合法"的近似样本，与规则 ID 对称分桶）两侧。
cargo test -p sieve-rules --release --test dataset_fp_rate -- --ignored --nocapture --test-threads=1

# 阈值（assertion 内嵌）：Critical 误报率守住产品公理上限、攻击样本召回守住下限。
# 输出按桶聚合，误报高时按桶定位是哪类合法场景误伤；漏检时按桶定位规则盲区。
```
...
- **真 P99 验证**：本地或 release 候选构建跑 `cargo bench --bench latency` 后用 criterion 报告查看 p99 行；P99 > 20ms 视为退化，由 reviewer 在 PR 中确认。
...
1. **Severity + FP 上限承诺** —— 对照误报率预算；申报 Critical 须承诺守住产品公理上限或更严格
...
2. **正则 / 词表匹配**
3. **校验位 / entropy / SHA-256**（如 BIP39 SHA-256 校验，核心硬约束）
4. **上下文关键词 + 占位符黑名单**
...
完整定义见 [docs/design/data-model.md](../design/data-model.md)。
...
```bash
sieve sieveignore add OUT-14:7a3b9c1d --comment "test mnemonic from unit-test fixture"
sieve sieveignore list
sieve sieveignore remove OUT-14:7a3b9c1d
```
...
> 行为序列检测（IN-SEQ-*）GA 默认关闭，由用户主动 opt-in。
...
> 注意：GA 发布的标准二进制**不含** `sequence_detection`，需要时单独编译提供。
...
- [ ] 架构变更必须同步 [`docs/design/architecture.md`](../design/architecture.md)
...
- [ ] 创建 `docs/specs/SPEC-NNN-<功能名>.md`（功能技术规格）
- [ ] 更新 `docs/design/architecture.md`（如涉及架构）
- [ ] 更新 `README.md` 功能列表
