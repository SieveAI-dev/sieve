...
| 候选       | 多模式正则吞吐                                     | 内存占用   | 启动时间        | 单二进制分发   | 维护成本 |
| -------- | ------------------------------------------- | ------ | ----------- | -------- | ------ |
...
| 测试 / fuzz            | `cargo test` + `cargo fuzz`（libFuzzer） | SSE 边界 fuzz 必需                                                                                  |
...
3. **招人面窄**：Rust 招人面相对窄；
4. **学习曲线**：lifetime / async / pin 这些概念是真实成本，Claude Code 在写 Rust 时的辅助能力可有效缓解；
