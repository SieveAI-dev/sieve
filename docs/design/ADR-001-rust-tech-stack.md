...
> 关联 PRD：[v1.4 §6.3、§9.1](../prd/sieve-prd-v1.5.md)
...
| JSON 流式解析            | `sonic-rs` + 自研 partial parser         | SIMD 加速；Anthropic SSE 中 tool_use 块是部分 JSON，需要自研 partial parser 配合                             |
| 客户端 HTTP（调上游）        | `reqwest`                              | 与 hyper / tokio 同生态；流式响应支持成熟                                                                  |
...
1. **性能达标**：vectorscan + sonic-rs 提供 SIMD 加速，是 P99 < 20 ms 唯一能算得过来账的栈；
...
- [PRD-sieve v1.4 §6.3](../prd/sieve-prd-v1.5.md) —— Rust 技术栈表
- [PRD-sieve v1.4 §9.1](../prd/sieve-prd-v1.5.md) —— 工程硬约束第 1 条："Rust 栈非选项"
