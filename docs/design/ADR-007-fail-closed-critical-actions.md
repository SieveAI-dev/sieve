...
UCSB+Fuzzland 论文《Your Agent Is Mine》已系统证明：上游模型 / 中转站 / prompt injection 任何一环被攻陷，YOLO mode 用户就是直接受害者。典型攻击链：
...
- Sieve 的产品定位（客户端最后一道闸）就是在不可逆动作前**强制打破这个信息不对称**。
...
**永远不可关闭**的 Critical 规则类别（任何配置 / 任何模式 / 任何 license 状态都强制生效）：

- 签名工具调用（EVM / Solana / Bitcoin 签名及发送交易）
- 危险 shell 模式（系统破坏、目录删除等不可逆操作）
- 远程脚本下载执行（`curl X | sh` 等模式）
- 编码后执行（base64 decode + eval 等绕过扫描的模式）
- 危险 shell 模式（crypto 上下文专项，如删除密钥目录）

具体检测规则的精确 pattern 与阈值不随本文档公开发布。

### 2. "永远不可关闭"的工程边界

| 边界 | 实现 |
|------|------|
| **配置层无法关闭** | `config.toml` 中对 Critical 规则的 `severity_overrides` 条目**启动时被忽略**并打印 `WARN`，不影响启动 |
| **CLI 标志无法关闭** | 不存在 `--disable-critical` / `--yolo` / `--no-prompt` 这类 flag；任何 PR 引入此类 flag CI hard-fail |
| **降级模式仍生效** | 降级触发 / license 失效 / 任何 license 异常状态下，Critical 规则**仍然 fail-closed** |
| **YOLO mode 仍生效** | Claude Code 的 YOLO 标志对 Sieve 不透传——Sieve 在自己的层独立判断，与上游 YOLO 状态正交 |
| **license 缺失仍生效** | 用户完全没装 license（trial 都没启动）的状态下，Critical 规则**仍然 fail-closed**（免费用户也享受 Critical 保护，是产品安全承诺的一部分） |
| **配置文件损坏仍生效** | 配置解析失败时启动时报错并拒绝启动；不存在"配置文件损坏 → fallback 到不开 Critical"的路径 |

规则加载失败或规则引擎编译失败均调用 `process::exit(1)`，**不降级为无规则运行**。

### 3. 持久化机制检测

入站检测涵盖攻击者常用的持久化路径，包括 shell rc 修改、定时任务配置、系统服务注册（launchctl / systemctl）、LaunchAgent plist 写入等类别。此类规则均为 Critical fail-closed，触发时阻塞并弹窗确认。

具体子规则枚举与精确 pattern 不随本文档公开发布。
...
- **`.sieveignore` 学习型白名单**：用户可对**非 fail-closed** 的 Critical 规则加 ignore；**但 fail-closed 规则不能加 ignore**——包含 fail-closed 规则 fingerprint 的 `.sieveignore` 条目启动时被忽略并打印 WARN；
...
- 在用户投诉"为什么你不让我关 Critical"时提供合规依据。
...
1. **0.001% 致命场景必救**：已知攻击模式与 drainer 手法被 fail-closed 捕获；
2. **产品定位兑现**："客户端最后一道闸"在工程层面真正成立，不是 marketing copy；
3. **抗社会工程攻击**：攻击者就算让用户被钓鱼到允许某次 prompt，也无法让 Sieve 整体关闭 Critical 拦截；
4. **抗用户侥幸心理**：很多用户会以为"我看看再说"然后忘了关回去——fail-closed 移除这个失误面；
5. **技术差异**：与可配置型方案不同，Sieve 对 Critical 规则不可关闭，这是产品层面的硬承诺而非可调项；
6. **简化客服**：用户问"怎么关 Critical 拦截"——回答永远是"不能关，这是产品承诺"，没有客服压力。

### 负面影响

1. **少数场景不适配**：批量执行长任务（如批量调试合约、批量重构）的用户会觉得每个签名都被拦很烦；这类批量执行场景不适合用 Sieve；
2. **可配置需求不覆盖**：偏好"完全可配置"的企业级需求不在当前产品范围内，本决策在该方向上有取舍，但不构成阻塞；
3. **edge case 误伤**：如某些合法的签名测试场景（hardhat / anvil 本地链）也会被弹窗——通过本地链 RPC URL 检测降级来缓解，**但仍然弹窗**，不会自动通过；
4. **维护负担**：fail-closed 规则的 FP 必须 < 0.5%（Critical FP 公理）——任何一条 FP 失控都会导致用户卸载。
...
|---------|------------|---------|
| **GUI 类**（地址替换 / 签名工具 / 通用危险标记）| `build_sieve_blocked_sse()` 截流注入 sieve_blocked | 保留 hold 流 + sieve_blocked 截流，语义改为"用户拒绝后的优雅终止" |
| **Hook 类**（危险 shell / 持久化机制等）| `build_sieve_blocked_sse()` 截流注入 sieve_blocked | 不再修改 SSE 流；仅写 `~/.sieve/pending/` 文件；fail-closed 由 sieve-hook 在 Claude Code PreToolUse 阶段完成（exit 1）|

Week 3 落地的 `build_sieve_blocked_sse()` 对 Hook 类规则的调用必须在 Week 5 删除。GUI 类调用保留。

原因：Hook 类 SSE 截流会把 tool_use 从 Claude Code 上下文中截断，导致上下文污染；且违反 v1.4 §9 第 11 条"不伪造协议字段"硬约束（详见 [ADR-014](./ADR-014-dual-layer-defense.md) §背景）。
