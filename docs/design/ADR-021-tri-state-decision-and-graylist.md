...
1. **体验摩擦**：合法操作反复弹窗导致用户习惯性点允许，降低高危场景的决策质量；
2. **无记忆**：没有"永久允许此次场景"的灰名单概念，无法区分"一次性允许"和"持久信任"。

### 关键风险：灰名单绕过 Critical

允许"永久绕过"的范围若包含内置 Critical 规则，等价于用户可以"关闭 Critical 拦截"，直接违反 fail-closed Critical 不可关的安全承诺。
...
| Ask（Hook 类，终端） | 否 | Hook 类规则命中，由 sieve-hook 终端 y/n 处理 | 内置 Critical 不可 Remember |
| Ask（GUI 类，Critical） | 否 | 内置 Critical GUI 类规则 | **❌ 强制 disabled** |
| Ask（GUI 类，非 Critical） | 可选 | 用户规则 / 非 Critical 入站通用规则 | ✅ |
...
3. 在名单内：忽略 remember 字段，写 audit ERROR + GUI 状态栏通知；
...
`~/.sieve/decisions/` 目录，每条灰名单一个 JSON 文件，文件名为指纹的 hex digest（不直接暴露 rule_id，防目录遍历泄露检测规则信息）：
...
灰名单有**指纹去重与完整性校验机制**：daemon 在读取灰名单文件时重新计算指纹，与文件内存储的指纹字段比对，不一致则忽略该文件并写 audit WARN，防止本地文件被人为编辑绕过。

具体的指纹计算配方（输入字段组合与归一化规则）不随本文档公开发布。

**灰名单查询**：daemon 在规则命中后、Decision 决策前查询 `decisions/` 目录。
...
| `graylist_integrity_fail` | 指纹校验不一致 | fingerprint / rule_id / file_path |
...
4. **指纹防伪**：灰名单文件被人为编辑时指纹校验失败，自动无效化，防止本地文件攻击。

### 负面影响

1. **灰名单过期策略未定**：`expires_at=null` 表示永久不过期，v2.0 暂不实现过期机制（v2.1 评估）。如用户换了上下文但历史灰名单仍生效，可能带来意外的自动允许；
2. **指纹规范化需严格一致**：SSE 流式 delta 拼接结果与 JSON full body 的规范化计算必须完全等价（否则写入的灰名单在另一路径查询时 miss）。需专项测试覆盖；
