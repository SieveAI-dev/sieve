...
1. **检测全部本地完成**：vectorscan + sonic-rs + entropy + checksum，全部在 Rust 进程内闭环；
...
2. **合规边界清晰**：完全本地 + 不上传 prompt → 不触发数据出境合规（PRD §11.5.3）；规则库下发是单向静态文件 + 签名 → 不构成"个人信息处理"；这让 doskey [redacted]的法律风险降到最低；
3. **离线可用**：用户在飞机 / 没网环境下，Sieve 仍然完整工作（除了 Anthropic 转发本身需要网络）；
4. **没有运维负担**：[redacted]没有维护 license server / telemetry pipeline / crash report backend 的能力——这个决策反过来变成"工程上的强制简化"；
