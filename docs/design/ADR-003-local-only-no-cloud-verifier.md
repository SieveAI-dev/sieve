...
| `updates.sieve.app`（域名占位,ADR-005 海外主体落地后实定，ADR-030 §3）| 拉规则更新 manifest（默认每 6h 一次,带 `?v=&os=&arch=&uid=&ch=` query;manifest 接口**不挂 CDN**,日志能追溯每次请求）| 启动时 + 6h 周期触发,即使内容无变化也照常发请求（兼装机量信标） |
