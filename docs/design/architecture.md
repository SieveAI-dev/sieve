...
> **依据 PRD**：docs/prd/sieve-prd-v2.0.md（当前权威源）；v1.5 归档见 docs/prd/sieve-prd-v2.0.md
...
| Phase 1 只适配 Anthropic，UnifiedMessage 接口预留 | 公理 7：不为想象用户写代码；第二适配等真实用户主动提                                                        |
| Sigstore 签名 + Reproducible Build + 透明日志   | 自证清白是产品定位（PRD §1.2 第 4 句），不只是工程实现                                                    |
...
> ⚠️ **历史说明**：v1.5.0~v1.5.3 实测的 FP 0% / Recall 99.71% **仅在 SSE 流响应模式有效**——非流式 `application/json` 响应路径曾存在入站检测缺口（v1.5.4 已修，详见 [SECURITY.md 历史 Advisories](../../SECURITY.md#历史-advisories)）。下表数字是 v1.5.4 后**所有响应模式**实测。
...
部分边界外样本受 vectorscan 能力限制，持续跟进。

### 5.2 规则引擎 stopwords 全文搜索机制（v1.5.1 新增）

> **v2.0 升级**：MatchEngine trait `scan(&[u8])` 接口将在 v2.0 Phase A 升级为 `scan(ScanRequest) -> ScanReport`，带上下文（direction / protocol / content_kind / tool_name / source_agent / caller_exe），让规则知道自己在哪条路径生效。LayeredEngine 包装系统规则 + 用户规则两层引擎，合并顺序保证用户规则不能 suppress 系统 Critical（PRD §9 #14）。
...
- ❌ Linux / Windows 平台支持（推 Phase 2，触发条件见 PRD §7.3）
- ❌ VS Code 插件 / 浏览器扩展
...
| Phase 2 能力           | 触发条件                                                           |
| -------------------- | -------------------------------------------------------------- |
| 二阶段轻量 ML 分类器         | 用户实际 High FP 持续 4 周 > 5%，**且**足够多真实用户反馈"误报太多"          |
| MCP 拦截（IN-MCP-01~03） | Week 16–20 启动；前提是 Phase 1 GA + 至少 1 个用户在 dogfood 中触发过 MCP 调用 |
| 协议白名单 + Drainer 黑名单  | Phase 2 数据合作落地后（慢雾 misttrack-skills / ScamSniffer Pro 接通）      |
| OpenClaw / Hermes 适配 | **真实用户主动要求时**（公理 7：不为想象用户写代码）                             |
...
**风险性质**：在 PRD §5.2「入站是 Sieve 真正的护城河」语境下，非流式路径的入站检测缺口
...
| 行为序列窗口 IN-SEQ-* | v2.0 Phase B（beta）| `ToolUseSequence` 滑动窗口 N=10/TTL=5min；仅 StatusBar 通知不阻断（feature `sequence_detection` 默认 off） |
