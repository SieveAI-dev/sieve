...
> GUI App 代码在独立仓库 `sieve-gui-macos`。两仓库唯一硬约束是 IPC 协议版本号（`sieve-ipc` crate，详见 [ADR-013](./ADR-013-ipc-protocol.md)）。
...
| ADR-004 | Phase 1 只适配 Anthropic，UnifiedMessage 接口预留 | 公理 7：不为想象用户写代码；第二适配等真实用户主动提                                                        | [ADR-004](./ADR-004-anthropic-first-unified-interface.md) |
