...
| ADR-005 | 海外公司主体作为收款与营销载体                           | 香港 → 新加坡 → Stripe Atlas；加密支付双通道                                                      | [ADR-005](./ADR-005-overseas-legal-entity.md)             |
...
> 这是"不做承诺，只做触发器"的原则——Phase 2 路线图的灵活性决定了一人公司在 12 周后能否快速响应真实用户。
...
- [ADR-005](./ADR-005-overseas-legal-entity.md) —— 海外公司主体
...
- `docs/api/api-reference.md` —— 待编写（Anthropic Messages API 适配细节 + UnifiedMessage 字段）
