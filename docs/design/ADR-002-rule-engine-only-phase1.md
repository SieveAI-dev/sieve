...
> 关联 PRD：[v1.3 §6.2](../prd/sieve-prd-v1.3.md)
...
1. **个人开发者对误报极其敏感**——任何一条 Critical FP 触发，用户就会卸载产品（[公理 12](../prd/sieve-prd-v1.3.md#65-误报率预算)，Critical FP > 0.5% → 用户禁用）；
...
本决策的可执行性依赖 [ADR-006](./ADR-006-sigstore-reproducible-build.md) 的 sigstore + reproducible build CI pipeline 在 Week 1 跑通——纯规则引擎方案不依赖外部 ML 服务，但每周规则更新（[PRD §8.3](../prd/sieve-prd-v1.3.md#83-规则更新)）必须依赖 Ed25519 签名 + Rekor 透明日志才不沦为投毒载体。两者是同一信任链的两个组件，缺一不可。
...
2. **规则集维护负担**：每周更新规则库（[PRD §8.3](../prd/sieve-prd-v1.3.md)）需要 doskey 持续投入 5–10 小时；用 Claude Code 辅助 + GitHub issue 公开样本流程缓解；
...
- [PRD-sieve v1.3 §6.2](../prd/sieve-prd-v1.3.md) —— 关键技术决策（v1.3 强化论证）
- [PRD-sieve v1.3 §6.5](../prd/sieve-prd-v1.3.md) —— 误报率预算（公理 12）
- [PRD-sieve v1.3 §9.1](../prd/sieve-prd-v1.3.md) —— 工程硬约束
