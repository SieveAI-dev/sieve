...
> 关联 PRD：[v1.4 §6.2](../prd/_archive/sieve-prd-v1.5.md)
...
但 Sieve 由小团队维护、资源受限，借助 Claude Code 推进，决策面临两个第一性原理硬约束：

1. **个人开发者对误报极其敏感**——任何一条 Critical FP 触发，用户就会卸载产品（[公理 12](../prd/_archive/sieve-prd-v1.5.md#65-误报率预算)，Critical FP > 0.5% → 用户禁用）；
2. **团队最稀缺的资源不是算力，是数据标注能力**——本地小模型的精度天花板取决于训练数据质量，团队无法独力支撑这个标注流程。
...
本决策的可执行性依赖 [ADR-006](./ADR-006-sigstore-reproducible-build.md) 的 sigstore + reproducible build CI pipeline 在 Week 1 跑通——纯规则引擎方案不依赖外部 ML 服务，但每周规则更新（[PRD §8.3](../prd/_archive/sieve-prd-v1.5.md#83-规则更新)）必须依赖 Ed25519 签名 + Rekor 透明日志才不沦为投毒载体。两者是同一信任链的两个组件，缺一不可。
...
- 一个能跑的"判断 prompt 是否包含敏感信息"的二分类器，**至少需要 5,000–10,000 条人工标注样本**（参考公开数据）；
- 持续维护需要每月新增 200–500 条标注（业务漂移）；
- 团队规模有限，无法额外承担数据标注流程这条战线。

**规则引擎绕开了这个瓶颈**：规则可以靠小团队 + Claude Code 持续维护，每条规则的"调优成本"是写一个测试用例 + 跑 benchmark，**数量级是分钟，不是月**。
...
- 团队自行 dogfood 时的过度乐观（"我觉得应该加 ML"）→ 没有真实用户驱动；
...
2. **规则集维护负担**：每周更新规则库（[PRD §8.3](../prd/_archive/sieve-prd-v1.5.md)）需要持续投入维护，用 Claude Code 辅助 + GitHub issue 公开样本流程缓解；
3. **纯文本意图识别能力弱于 ML 方案**：规则引擎在"这段 prompt 像不像在套取我的 API key"这类语义判断上覆盖弱；权衡是个人开发者更在意的是"不烦人 + 不出事"，对应 PRD §1.2 第 2 句"没人能替你兜底"；
...
- [PRD-sieve v1.4 §6.2](../prd/_archive/sieve-prd-v1.5.md) —— 关键技术决策（v1.3 强化论证）
- [PRD-sieve v1.4 §6.5](../prd/_archive/sieve-prd-v1.5.md) —— 误报率预算（公理 12）
- [PRD-sieve v1.4 §9.1](../prd/_archive/sieve-prd-v1.5.md) —— 工程硬约束
