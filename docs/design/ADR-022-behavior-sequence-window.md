...
> 关联 PRD：v2.0 §5.7、§9 #15
> 关联 codex review：2026-05-01 codex review §C2、§E1/E2、Should #1、Must #5
> 关联风险：R-V20-02 行为序列 FP 失控、R-V20-08 行为序列串会话
...
v1.5 单次检测无法识别多步操作链：当前已有框架概念但无序列联动实现。
...
闭测用户 + dogfood 用户主动将 `sequence_detection = true` 配置开启。行为序列检测 GA 默认关闭，需显式 opt-in。
...
1. **HIPS 能力补完**：行为序列检测补完单次检测在多步 kill chain 上的盲区，Phase B 实现后弥补此前缺失的序列联动能力；
...
1. **GA 默认关闭**：行为序列是 v2.0 HIPS 升级的能力之一，但 GA 默认关闭、需 opt-in，待 v2.1 数据充足后再评估是否默认开启；
...
- PRD v2.0 §5.7 —— 行为序列联动完整需求（滑动窗口 + 三条启发式 + 双路径不变量）
- PRD v2.0 §9 #15 —— 保守起步 + beta 默认关闭硬约束
- PRD v2.0 §9 #16 —— content-type 路由矩阵测试硬约束（双路径不变量的测试要求来源）
- PRD v2.0 §12 R-V20-02、R-V20-08 —— 行为序列 FP 失控 + 串会话风险
- codex review 2026-05-01 §C2、§E1/E2、Should #1 —— 结构化特征 + 默认关闭 + 双路径不变量的依据
...
- HIPS Readiness Assessment 2026-05-01 —— v2.0 改造出发点（行为序列为"部分满足"项）
