...
> 关联 PRD：[v2.0 §6.5 IPC 协议、§6.6 GUI App](../prd/sieve-prd-v2.0.md)
...
[redacted] + 双语言 + GA 前阶段，拆 crate / 拆仓属于过度设计（YAGNI）。**触发条件发生时再写一个新 ADR 决策升级，本 ADR 不预判。**
...
- [PRD v2.0 §6.5 IPC 协议、§6.6 GUI App](../prd/sieve-prd-v2.0.md)
...
- [ADR-021 三态决策 + 灰名单](./ADR-021-three-way-decision-graylist.md) —— 三态决策语义跟 client 形态无关，本 ADR 不影响
- [ADR-026 Port-based listener routing](./ADR-026-port-listener-routing.md) —— 同期推进的 unix-style 改造，listener 与 IPC 协议各管各，独立可交付
