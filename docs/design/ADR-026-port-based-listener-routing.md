...
> 决策日期：2026-05-05（2026-06-07 补充 `Protocol::Auto` 默认态语义，见 §决策 4 补充说明）
> 范围：Phase 2.x daemon（v2.x 工程项，不阻塞 GA；GA 前完成）
> 关联 PRD：v2.0 §6.1 部署拓扑、v2.0 §6.5 IPC 协议
...
> **补充说明（2026-06-07，向后兼容修订，不推翻原决策）**：
>
> `Protocol` 枚举新增第三态 `auto`（**默认值**，对应未显式声明协议的 listener）。语义如下：
>
> - **`auto` listener**（含 legacy `upstream_url`、省略 `protocol` 字段的 `[[upstream]]`）按请求 path 自适应路由（`/v1/messages` → Anthropic 处理，`/v1/chat/completions` → OpenAI 处理），**不做协议错位拒绝**，保留单 upstream 双协议能力（恢复 v1.x 行为）。此举是为兑现本 ADR §决策 1 的「向后兼容」承诺，同时满足 PRD §9 #16（content-type 路由矩阵）/ #9（双协议硬约束）。
> - **仅当 listener 显式声明 `protocol = "anthropic"` 或 `"openai"` 时**，才触发上文 §决策 4 的 path 错位 fail-closed 400（该 fail-closed 行为对显式声明者完全保留，不放宽）。
>
> 即：错位 400 是**显式声明者的安全契约**，而非对所有 listener 的默认行为。背景见 [CHANGELOG 2026-06-07 Fixed 条目](../changelog/CHANGELOG.md)。
...
- PRD v2.0 §6.1 部署拓扑
- PRD v2.0 §6.5 IPC 协议
- PRD v2.0 §9 #2 完全本地 —— bind_addr 不可改不变量
- PRD v2.0 §9 #16 content-type 路由矩阵 —— 4 类组合 × N listener 测试硬约束
- [ADR-013](./ADR-013-ipc-protocol.md) —— IPC 协议（本 ADR 的 listener routing 不影响 IPC，UDS 仍单一 `~/.sieve/ipc.sock`）
- [ADR-018](./ADR-018-openai-protocol-adaptation.md) —— OpenAI 协议适配（本 ADR 的 `protocol` 字段是 ADR-018 引入的双协议在 listener 层的显式化）
