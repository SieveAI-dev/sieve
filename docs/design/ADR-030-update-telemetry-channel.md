...
### 2026-05-05 域名确定 + crate 归属落地

1. **域名 = `sieveai.dev`** ✅(2026-05-05)—— 子域 `updates.sieveai.dev`(manifest,**不挂 CDN**)/ `cdn.sieveai.dev`(规则正文 zst)/ `security@sieveai.dev`(漏洞报告渠道)。DNS / MX 注册待 [ADR-005](./ADR-005-overseas-legal-entity.md) 海外主体落地后执行。
2. **客户端 crate 归属 = 新增 `sieve-updater` 独立 crate** ✅(2026-05-05)—— 见 workspace `Cargo.toml` 第七个 crate;职责清晰,GUI 仓后续可复用同一份 updater 模块。

## 待决项(GA 前必须落地)

1. ~~**域名**~~ ✅ 已确定 = `sieveai.dev`(2026-05-05),见上方「决策落地」段
...
5. ~~**客户端 crate 归属**~~ ✅ 已确定 = 新增 `sieve-updater` 独立 crate(2026-05-05),见上方「决策落地」段
