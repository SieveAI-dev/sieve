...
- 修复多客户端连接下决策请求可能被「连接仍在、对端已不消费」的僵尸连接吞掉、确认弹窗永不出现的问题（GUI 重启过渡期真机实证）：决策请求改为广播给所有已连接客户端，首个应答生效，其余客户端自动收到取消通知收回弹窗。
- 修复决策等待期间发起方连接断开（如 curl 被中断）导致待决策条目滞留（`sieve decisions list` 出现远超超时时长的僵尸条目）、且不通知客户端收回弹窗的问题：断连即时清理并广播取消通知。
...
- 修复用户决策审计事件方向恒记 inbound 的问题：出站规则（如 PEM 私钥弹窗）的允许/拒绝决策现记录真实方向 `outbound`，`sieve audit query` 输出的 direction 列不再对出站决策恒错；此前写入的旧记录读取时保持兼容（缺失字段回退 `inbound`）。
...
- 修复 CI「ga_keys 编译期 gate 有效性」job 自真实公钥嵌入源码后永久红的误报——**原因**：旧判定直接断言「树内密钥下 `--features ga_keys` 编译失败」，等价于断言树内密钥仍是占位，真公钥就位后断言永久反向失败，只能靠 `continue-on-error` 存档口径。改为正负两向自足判定（真公钥必须编译成功 + 临时替换回占位密钥必须被 gate 的 const assert 拒绝，且校验失败原因就是 gate 断言本身），不再依赖树内密钥状态，摘除 `continue-on-error` 恢复为硬门禁。gate 本身经本地正负两向实测一直有效，无安全回归。
