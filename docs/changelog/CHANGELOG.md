...
- **Phase 1（GA 后 6–12 个月）改为纯免费,装机量为唯一指标**——原 PRD §7「14 天试用 + $49/月正式版 + 降级模式」定价模式被 ADR-029 替换。
- **永久排除「中转站认证 + 排名广告」收费模式**（评级机构利益冲突会摧毁「客观本地审计」产品定位）。
- Phase 2+ 商业化方向预备清单（用户端 Pro 订阅 / 中转站 B 端订阅 / 客观技术认证,**认证与广告严格分离**），具体路径待 6 个月装机数据出来后定。
- ~~18 个月 MRR ≥ $25K~~ → 装机量阈值优先,MRR 目标推后到 Phase 2。
...
- **运维侧**：域名 `updates.sieveai.dev` / `cdn.sieveai.dev` 注册（待 ADR-005 海外主体落地）/ ed25519 签名密钥管理（HSM / 单独 build 机 / GCP KMS 之一,写入 ADR-006 follow-up）/ 服务端实现（**倾向 Cloudflare Workers + KV / D1**,manifest 接口天然反 DDoS）/ ch 通道策略（首发 stable 单通道,Phase 2 加 beta）。
...
- **SPEC-003 §4.2b doctor multi-listener 体检 + SPEC-004 §4.2.6 header vs port
  ...
  3 份文档同步 +135 行；Pro Mode（ADR-027）前向引用。
...
依据 [HIPS Readiness Assessment](../review/_archive/2026-05-01-hips-readiness-assessment.md) 评估"Sieve 当前 HIPS 70%"，启动**完整 HIPS（Host-based Intrusion Prevention System）改造**。目标 GA 时达到 HIPS 90%。GA Week 12 时间表不变，Phase A（Week 5-8）+ Phase B（Week 9-12）拆分。
...
- ADR-005 海外公司注册 Week 1 启动（非工程任务，doskey 跟进）
...
- 关键改动：
  - 定价收敛到 **单一 $49/月**（年付 $490），降级模式只读警告（公理 11 / 12）
...
| 8   | **用户教育成本作为风险登记** + 海外公司注册周期延误风险                                                  | §12        |


附加改动：

- §1.4 法律实体明确：海外注册（首选香港，次选新加坡）
- §3.1 P0 客群地理分布：海外为主
...
- [ADR-005](../design/ADR-005-overseas-legal-entity.md) —— 移除未授权的 BVI / Cayman 离岸主体行（PRD §1.4 仅锁定香港 / 新加坡 / Stripe Atlas）
