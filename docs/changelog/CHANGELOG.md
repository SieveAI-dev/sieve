...
- **运维侧**：域名 `updates.sieve.app` / `cdn.sieve.app` 注册（待 ADR-005 海外主体落地）/ ed25519 签名密钥管理（HSM / 单独 build 机 / GCP KMS 之一,写入 ADR-006 follow-up）/ 服务端实现（**倾向 Cloudflare Workers + KV / D1**,manifest 接口天然反 DDoS）/ ch 通道策略（首发 stable 单通道,Phase 2 加 beta）。
