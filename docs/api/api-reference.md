...
**出站 AutoRedact（OUT-01~05/12/13 + OUT-14 BIP39 助记词）**：
- 请求 body 中敏感内容自动脱敏替换，**不返 426**，直接转发到上游，上游响应直通
- 状态栏静默通知（不打断用户流程）
- BIP39 助记词（`OUT-14`，SHA-256 checksum 通过才触发）同属此类：命中窗口改写为 `[REDACTED:OUT-14]` 占位符后 `200` 转发。历史版本曾以 `OUT-09` 编号对 BIP39 做 GuiPopup + 426 硬阻断，现已改为自动脱敏（`OUT-09` 编号现属 Slack Token 规则，处置为 GuiPopup，见下）

**出站 GuiPopup（OUT-06/07/08/09/10）**：
- hold 出站请求，弹 HIPS 弹窗等用户确认（Sieve 差异化点，普通 DLP 工具不做这一步）
- 用户允许 → 原文转发；用户拒绝 → 返 `426 Upgrade Required` + `sieve_blocked` JSON
- 分档：OUT-07/10（PEM / OpenSSH 私钥）强警告，60s 超时默认拒绝；OUT-06/08（JWT / Stripe live key）15s 超时默认脱敏后发送；OUT-09（Slack Token）fail-closed（不可关闭、不可永久白名单），60s 超时默认拒绝
...
```json
{
  ...
  "severity": "critical",
  "rule_id": "OUT-07",
  "rule_name": "PEM-encoded private key",
  "fingerprint": "OUT-07:7a3b9c1d",
...
audit_events 表（表名口径见 [data-model.md §6.2](../design/data-model.md)，与本文 §6.4b 一致）通过 v2 migration（commit cd0248d）自动添加两列：
...
{ "fingerprint": "OUT-14:7a3b9c1d", "comment": "test mnemonic from unit-test fixture" }
```

响应：

```json
{ "added": true, "fingerprint": "OUT-14:7a3b9c1d", "added_at": 1745683210000 }
```
...
```json
{ "removed": true, "fingerprint": "OUT-14:7a3b9c1d" }
```
...
{
  "entries": [
    { "fingerprint": "OUT-14:7a3b9c1d", "comment": "...", "added_at": 1745683210000 }
  ]
}
```

> fingerprint 格式 = `<rule_id>:<sha256_prefix_16_hex>`，例如 `OUT-14:7a3b9c1d5e6f7890`。`sha256_prefix` 取规则匹配内容 SHA-256 的前 **8 字节，以 lowercase hex 编码为 16 个字符**——足以在单用户审计库内唯一标识，且不暴露原文。详见 [data-model.md](../design/data-model.md)。
...
"OUT-14" = "high"     # ❌ 启动失败：OUT-14（BIP39 助记词）默认 critical，禁止降级
```
...
|-------------|---------|---------|----------------|------|
| **AutoRedact** | OUT-01~05/12/13/14 | 出站 | `200 OK` + 改写后的请求 body 转发上游；上游响应**直通**，**不返 426** | 自动脱敏，不打断用户流程；OUT-12/13（WIF/xprv）经 Base58Check second-pass 校验和验证后脱敏；OUT-14（BIP39 助记词）经 SHA-256 checksum second-pass 验证后脱敏为 `[REDACTED:OUT-14]`（历史版本曾以 OUT-09 编号 + GuiPopup/426 处置，已改） |
| **GuiPopup**（出站） | OUT-06/07/08/09/10 | 出站 | hold 请求；GUI 弹窗；用户允许 → `200 OK` + 原文/脱敏后转发；用户拒绝 → `426 Upgrade Required` + `sieve_blocked` JSON | OUT-07/10（PEM / OpenSSH 私钥）高确定性私钥强警告（60s 超时默认拒绝），Sieve 差异化点；OUT-06/08（JWT / Stripe live key）15s 超时默认脱敏后发送；OUT-09（Slack Token）fail-closed，60s 超时默认拒绝 |
...
| `OutboundRedacted` | 出站脱敏（OUT-01~05/12/13/14 AutoRedact 命中，静默通知 5s） |
...
| ----------------------------------- | ----------------- | -------------------------------------------------------------------------- | ------------------------ |
| `426 Upgrade Required`              | **Critical 出站阻断** | 出站请求体命中 GuiPopup 类规则且用户拒绝或超时默认拒绝（如 OUT-07 PEM 私钥；AutoRedact 类不返 426）                            | `sieve_blocked` JSON     |
