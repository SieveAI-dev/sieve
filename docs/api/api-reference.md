...
> **不存在云端 API。**Sieve 完全本地运行，绝不联网做 token verification（[PRD §9](../prd/sieve-prd-v2.0.md) 硬约束 #2）。
...
> 上游真实 endpoint 由 `[upstream].url` 决定（默认 `https://api.anthropic.com`），用户可指向中转站。
...
- 状态栏静默通知（不打断用户流程，[PRD §9 第 13 条](../prd/sieve-prd-v2.0.md)）
...
> **绝不返回原文。**只返回 fingerprint + 元信息（[PRD §9](../prd/sieve-prd-v2.0.md) 硬约束 #2 + §11.3 数据本地化）。详细字段定义见 [data-model.md](../design/data-model.md)。
...
- Critical 在所有版本（含降级/试用过期模式）不可关闭（[PRD §9 #8](../prd/sieve-prd-v2.0.md)）；降级模式下 High 仅审计记录，但 Critical 的 GuiPopup/HookTerminal 行为不变
