...
| `SIEVE_NO_UPDATE` | sieve 进程 | 否 | 未设置 | 完全跳过 manifest 更新检查（不发请求,规则冻结,无遥测）—— 离线 / 隔离网络 / CI 测试。命中时启动 banner 必打印 `update check disabled by SIEVE_NO_UPDATE`。详见 ADR-030 §5 |
...
> manifest 接口是 Sieve 唯一允许的主动出站接口（详见 [ADR-003 amended](../design/ADR-003-local-only-no-cloud-verifier.md) + ADR-030）。完整协议规格见 [SPEC-006](../specs/SPEC-006-update-and-telemetry.md)。
...
详见 [SPEC-006 §5 签名校验](../specs/SPEC-006-update-and-telemetry.md) 和 ADR-030 §3。
