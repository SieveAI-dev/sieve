...
> **不存在云端 API。**Sieve 完全本地运行，绝不联网做 token verification（PRD §9 硬约束 #2）。
...
- `sieve.request_decision` 新增字段：
  - `allow_remember: bool` —— **daemon 端计算**该值（不让 GUI 决定）。当 `rule_id` 在 `critical_lock::FAIL_CLOSED_RULES` 时强制 false（PRD §9 #3，fail-closed 不被绕过）
...
- **降级模式不影响入站 Critical**：降级模式用户的入站 Critical 仍然 fail-closed（PRD §9 #8）
...
> **绝不返回原文。**只返回 fingerprint + 元信息（PRD §9 硬约束 #2 + §11.3 数据本地化）。详细字段定义见 [data-model.md](../design/data-model.md)。
...
> 见 [SPEC-009](../specs/SPEC-009-encrypted-audit-log.md)（Phase 2 落地）。三档 logging level 控制 daemon 落盘审计行为；`full` 档为 opt-in 加密归档（write-only logging）。
...
### 3.3.4 超额计费检测配置（`[billing_check]` + `[[upstream]].trust`）

> 见 [SPEC-010](../specs/SPEC-010-overbilling-detection.md)（Phase 2 落地）。对经过的 LLM 流量做独立 token 核算，交叉比对中转站（relay）声明的 `usage`，偏差超容差报警（仅 StatusBar 通知，**不阻断流量**）。统计严格本地、**永不上传**（呼应 [SPEC-006 §9.1](../specs/SPEC-006-update-and-telemetry.md)）。
...
- Critical 在所有版本（含降级模式）不可关闭（PRD §9 #8）；降级模式下 High 仅审计记录，但 Critical 的 GuiPopup/HookTerminal 行为不变
...
> 配套 `full` 档加密归档（§3.3.3 `[audit]` 段）。这三个子命令是 `full` 档的密钥生命周期工具——daemon 端只持公钥（write-only logging），解密 / 审计须用持口令的私钥（identity），**应在另一台 / 离线机器执行**。详见 [SPEC-009](../specs/SPEC-009-encrypted-audit-log.md)。
...
> 本章对应 PRD v2.0 §6.5，描述 sub-agent 嵌套调用链的 header 协议。
...
- **冻结时间点**：Week 12 GA（参见 PRD §10.2 Week 12）
