...
- 上游真实 endpoint 由配置 `[upstream].url` 决定（默认 `https://api.anthropic.com`），用户可指[redacted]
...
> 选用 `426` 是产品语义层的"行为升级要求"（用户需脱敏后重发），与 RFC 7231 协议升级语义有偏差，但**实际客户端不会因此崩溃**（Anthropic SDK 把非 2xx 当作错误并显示 body 文本）。**ADR-008 状态**：候选，Week 2 dogfood 验证 Claude Code SDK 行为后正式落 ADR（见 [ADR-INDEX](../design/ADR-INDEX.md)）。
