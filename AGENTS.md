...
Sieve 是一个完全本地运行的 LLM 流量代理（Rust 单二进制），夹在 Codex 和 Anthropic API 之间，对 crypto 开发者做双向安全检测，在不可逆动作（签名 / 转账 / 部署）前强制插入认知摩擦。
...
1. **PRD v1.5** — [docs/prd/sieve-prd-v1.5.md](docs/prd/sieve-prd-v1.5.md)（已锁定执行，唯一权威源）
...
- `docs/requirements/PRD-sieve.md` 是 v1.5 的薄指针，不复制全文
- `docs/prd/` 下 v1.0~v1.4 是历史归档，**永不修改**
...
9. **Phase 1 GA 适配三家：Codex / OpenClaw / Hermes** —— UnifiedMessage 真实运行时支持 Anthropic Messages API + OpenAI Chat Completions 双协议；其他协议（Gemini / Mistral 等）推 Phase 2
...
候选 ADR：ADR-008（426 状态码）、ADR-009（Windows 服务）、ADR-010（加密支付通道）。已接受：ADR-012（Native GUI App）、ADR-013（IPC 协议）、ADR-014（双层防御）、ADR-015（sieve setup 工具）、ADR-016（处置矩阵二维化）、ADR-018（OpenAI 协议适配）、ADR-019（X-Sieve-Origin header）。
...
2. **`sieve setup` 一键安装**：检测 Codex → 注册 PreToolUse hook + 写 ANTHROPIC_BASE_URL → 加载 launchd plist；朋友 30 分钟内能装通（[SPEC-003](docs/specs/SPEC-003-sieve-setup-tool.md)）
...
- **doskey 一人 + Codex**，不要假设有团队 / DevRel / 销售 / 数据标注师
- **[redacted] + Stripe + [redacted]**，[redacted]（[ADR-005](docs/design/ADR-005-overseas-legal-entity.md)）
- **[redacted] to-C 公开商业化营销**——Twitter / Hacker News / Mirror 是主战场，微信 / 小红书 / 知乎 / B 站不规划（[PRD §11.5.2 渠道分级](docs/prd/sieve-prd-v1.5.md#1152-营销渠道分级)）
