...
15. **行为序列检测保守起步 + beta 默认关闭**（v2.0）—— IN-SEQ-* 仅触发 StatusBar 通知，不引入新 Block 路径；GA 默认关闭（feature flag `sequence_detection = false`），闭测用户主动 opt-in；GA 营销不承诺行为序列。升级为 Block 类需 4 周 ≥ 50 样本 + FP < 0.5% + 新 ADR。
