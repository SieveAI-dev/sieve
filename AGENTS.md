...
Sieve 是一个完全本地运行的 LLM 流量代理（Rust 单二进制），夹在 AI 编码 agent（Phase 1 适配 Claude Code / OpenClaw / Hermes）和上游模型之间，对 crypto 开发者做双向安全检测，在不可逆动作（签名 / 转账 / 部署）前强制插入认知摩擦。

## 项目状态

**v2.0 + v2.1 代码 100% 落地（2026-05-01）— 进入 dogfood 准备阶段**。Cargo workspace 6 crate（`sieve-core` / `sieve-rules` / `sieve-policy` / `sieve-cli` / `sieve-ipc` / `sieve-hook`）+ fuzz crate；sigstore CI pipeline 在跑。源码在 `crates/` 下，文档与代码已对齐。
...
2. **ADR** — [docs/design/ADR-INDEX.md](docs/design/ADR-INDEX.md)（21 个已接受 + 候选 ADR-008/009/010）
...
全局 AGENTS.md 只覆盖 Python/JS/Shell，Rust 规范全部在本文 + .cursorrules：
...
用户级 [~/.Codex/AGENTS.md](~/.Codex/AGENTS.md) 已设定通用规则：沟通风格、工作流（规划 / 验证 / Bug 修复 / 经验沉淀）、子代理调度策略、Python/JS/Shell 规范、Git 提交规范（含禁止 AI 签名）、文档管理 v2.0 + DOCS-STANDARD 引用、RTK 命令代理。
